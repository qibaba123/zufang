 ...
 * Time: 下午5:30
 */
class App_Model_Unitary_MysqlCfgStorage extends Libs_Mvc_Model_BaseModel {

    private $sid;//店铺id
    private $shop_table = '';
    public function __construct($sid) {
        parent::__construct();
        $this->_table   = 'unitary_cfg';
        $this->_pk      = 'uc_id';
        $this->_shopId  = 'uc_s_id';
    ...
    public function getShopList($where,$index,$count,$sort){
        $sql = 'select  uc.*,s_name,s_id ';
        $sql .= ' from `'.DB::table($this->_table).'` uc';
