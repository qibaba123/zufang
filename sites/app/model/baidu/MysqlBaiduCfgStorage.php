 ...
 * Time: 上午10:39
 */
class App_Model_Baidu_MysqlBaiduCfgStorage extends Libs_Mvc_Model_BaseModel {

    private $sid;//店铺id
    private $shop_table;
    public function __construct($sid = null) {
        parent::__construct();
        $this->_table   = 'applet_baidu_cfg';
        $this->_pk      = 'ac_id';
    ...
    public function getShopAppletListMember($where,$index,$count,$sort){
        ...
        $sql .= ' left join pre_agent_admin aa on aa.aa_id=ao.ao_a_id ';
        $sql .= ' left join (select t_s_id,sum(t_total_fee) as tradeTotal,count(t_id) as tradeCount from pre_trade where t_status in (3,4,5,6,8) group by t_s_id) as trade on trade.t_s_id = ac.`ac_s_id` ';
        $sql .= ' left join (select m_s_id,count(m_id) as memberCount from pre_member where m_source = 4 group by m_s_id) as member on member.m_s_id = ac.`ac_s_id` ';
//        $sql .= ' left join (select ac2.ac_s_id,count(*) as memberCount from pre_applet_cfg ac2 left join pre_member m on m.`m_s_id` = ac2.`ac_s_id` '.$this->formatWhereSql($where).' GROUP BY ac2.ac_s_id '.$this->formatLimitSql($index,$count).' ) mc on ac.`ac_s_id` = mc.ac_s_id ';
//        $sql .= ' left join pre_trade t on ac.`ac_s_id` = t.`t_s_id` AND t.`t_status` in (3,4,5,6,8) ';
//        $sql .= ' left join pre_member member on s.s_id = member.m_s_id ';
        $sql .= $this->formatWhereSql($where);
        $sql .= ' GROUP BY s_id ';
        $sql .= $this->getSqlSort($sort);
        $sql .= $this->formatLimitSql($index,$count);
