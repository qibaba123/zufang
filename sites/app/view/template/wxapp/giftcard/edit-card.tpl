...
<script>
    ...
    app.controller('chCtrl',['$scope','$http','$timeout', function($scope,$http,$timeout){
        ...
        $scope.cover       = '<{$row['agc_cover']}>'?'<{$row['agc_cover']}>':'/public/manage/img/zhanwei/zw_fxb_750_422.png';
        $scope.name        = '<{$row['agc_name']}>' ? '<{$row['agc_name']}>': '';
        $scope.rule        = '<{$row['agc_rule']}>' ? '<{$row['agc_rule']}>': '';
        $scope.price        = '<{$row['agc_price']}>' ? '<{$row['agc_price']}>': '';
        $scope.coin        = '<{$row['agc_coin']}>' ? '<{$row['agc_coin']}>': '';
        $scope.sort        = '<{$row['agc_sort']}>' ? '<{$row['agc_sort']}>': '0';
        $scope.cardId      = '<{$row['agc_id']}>' ? '<{$row['agc_id']}>' : '0';
        $scope.cardType    = '<{$cardType}>' ? '<{$cardType}>' : '1';
