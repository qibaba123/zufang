...
<div class="password-redpack">
    ...
    <form id="keyword-redpack">
        <table class="input-table">
            <tr>
                ...
                <td>
                    <span class='tg-list-item'>
                        <input class='tgl tgl-light' id='video_is_open' type='checkbox' onclick="showCfg()" <{if $row && $row['av_is_open']}>checked<{/if}>>
            ...
            <tr class="cfg-row" <{if !row  || $row['av_is_open'] == 0}>style="display:none;"<{/if}> >
                <td class="label-td"><label><span class="red">*</span>视频链接:</label></td>
                <td>
                    <div><textarea rows="3" id="videoUrl" name="videoUrl" class="form-control" placeholder="请填写视频链接"><{if $row && $row['av_video_url']}><{$row['av_video_url']}><{/if}></textarea>
            ...
            <tr class="cfg-row" <{if !row  || $row['av_is_open'] == 0}>style="display:none;"<{/if}> >
                ...
                <td>
                    <div>
                        <input type="number" id="videoTime" style="height: 34px;" name="videoTime" value="<{if $row && $row['av_time']}><{$row['av_time']}><{/if}>" class="form-control" placeholder="请填写视频时长单位秒，2分15秒则填写135" />
