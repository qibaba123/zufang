		    	...
		    	<div>
		    		<form action="">
						<div class="form-group">
							<label for="firstname" class="col-sm-2 control-label text-right">所属地域<font color="red">*</font></label>
							<div class="col-sm-10">
								<input type="text" class="form-control" id="zone" value="<{if $tencentyun}><{$tencentyun['at_bucket_zone']}><{/if}>" data-msg="填写您的存储区域，并保存" placeholder="填写您的存储区域">
								<p>请填写存储区域，例：ap-beijing</p>
							</div>
						</div>
						<div class="form-group">
							<label for="firstname" class="col-sm-2 control-label text-right">空间名称<font color="red">*</font></label>
							<div class="col-sm-10">
								<input type="text" class="form-control" id="name" value="<{if $tencentyun}><{$tencentyun['at_bucket_name']}><{/if}>" data-msg="填写您的存储空间名称，并保存" placeholder="填写您的存储空间名称">
								<p>请填写存储空间名称</p>
							</div>
						</div>
						<div class="form-group">
							<label for="firstname" class="col-sm-2 control-label text-right">SecretId<font color="red">*</font></label>
							<div class="col-sm-10">
								<input type="text" class="form-control" id="secretId" value="<{if $tencentyun}><{$tencentyun['at_secret_id']}><{/if}>" data-msg="填写您的SecretId，并保存" placeholder="填写您的SecretId">
								<p>请填写SecretId</p>
							</div>
						</div>
						<div class="form-group">
							...
							<div class="col-sm-10">
								<input type="text" class="form-control" id="secretKey" value="<{if $tencentyun}><{$tencentyun['at_secret_key']}><{/if}>" data-msg="填写您的SecretKey，并保存" placeholder="填写您的SecretKey">
								<p>请填写SecretKey</p>
							</div>
						</div>
						<div class="form-group">
							<label for="firstname" class="col-sm-2 control-label text-right">访问域名<font color="red">*</font></label>
							<div class="col-sm-10" style="position: relative;">
								<input type="text" class="form-control" id="host" value="<{if $tencentyun}><{$tencentyun['at_host']}><{/if}>" data-msg="填写您的访问域名，并保存" placeholder="填写您的访问域名">
								<p>请填写访问域名</p>
							</div>
						</div>
...
<script>
	function saveQiniu(){
		var check = new Array('zone','name','secretId','secretKey','host');
		var data  = {};
		...
		$.ajax({
			'type'  : 'post',
			'url'   : '/wxapp/currency/saveTencentyun',
			'data'  : data,
			'dataType'  : 'json',
