<link rel="stylesheet" href="/public/wxapp/share/css/index.css">
            ...
            <div class="index-con">
                ...
                <div class="index-main">
                    <div class="post-wrap cur-edit" data-left-preview>
                        <div class="post-img">
                            <img id="previewImg" src="<{if $row && $row['ac_share_addr']}><{$row['ac_share_addr']}><{else}>/public/wxapp/share/images/haibao.png<{/if}>" alt="海报图片" >
                        </div>
                    </div>
                    <div class="opera-btn">
...
top: 10px;">
                        <input class='tgl tgl-light' id='share_custom' type='checkbox'  <{if $row && $row['ac_share_custom'] == 1}>checked<{/if}>>
                ...
                <div class="input-group" style="margin-bottom: 10px;">
                    ...
                    <span class="span-right" style="width: 300px"><input type="text" class="cus-input" placeholder="请输入分享标题" id="share_title" maxlength="30" value="<{if $row && $row['ac_share_title']}><{$row['ac_share_title']}><{/if}>"></span>
                </div>
                <div class="poster-img" style="margin-bottom: 10px;">
                    ...
                    <div class="cropper-box" data-width="500" data-height="400" >
                        <img style="width: 40% !important;" src="<{if $row && $row['ac_share_cover']}><{$row['ac_share_cover']}><{else}>/public/manage/img/zhanwei/zw_555_480.png<{/if}>" onload="" />
                        <input type="hidden" id="share_cover" name="cover" value="<{if $row && $row['ac_share_cover']}><{$row['ac_share_cover']}><{/if}>" />
                    </div>
                </div>
                <{if $row['ac_type'] == 33 || $row['ac_type'] == 34}>
                <label>海报宣传图片<span>（图片建议尺寸600px*600px）</span></label>
                <{elseif $row['ac_type'] != 30}>
                <label>海报宣传图片<span>（图片建议尺寸600px*855px）</span></label>
                <{else}>
                <label>海报宣传图片<span>（图片建议尺寸600px*600px）</span></label>
                <{/if}>
                <div class="poster-img">
                    <div class="cropper-box" data-width="600" <{if $row['ac_type'] == 33 || $row['ac_type'] == 34}>data-height="600"<{elseif $row['ac_type'] != 30}>data-height="855"<{else}>data-height="600"<{/if}>>
                        <img src="<{if $row && $row['ac_share_addr']}><{$row['ac_share_addr']}><{else}>/public/wxapp/share/images/haibao.png<{/if}>" onload="imgSrcChange(this)" />
                        <input type="hidden" id="share_addr" name="img" value="<{if $row && $row['ac_share_addr']}><{$row['ac_share_addr']}><{/if}>" />
                    </div>
                </div>
            </div>
        </div>
