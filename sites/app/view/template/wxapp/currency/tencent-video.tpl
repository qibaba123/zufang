...
<div id="content-con">
    ...
    <div class="authorize-tip flex-wrap">
        <div class="shop-logo">
            <img src="/public/wxapp/images/tencent-icon.jpg" alt="logo">
        </div>
        <div class="flex-con">
            <h4>腾讯云对象存储</h4>
        </div>
        <div class="opera-btn-box"><a href="javascript:void(0)" onclick="changeServer(this,event)" class="btn btn-sm btn-cyan">切换七牛云</a></div>
    </div>
    <div class="authorize-tip flex-wrap">
        <div class="shop-logo">
        ...
        <div class="flex-con">
            ...
            <p class="state" style="color: #999;">
                <span>您已配置腾讯云信息，修改时会有5分钟左右的延迟，请稍后刷新重试</span>
            </p>
            <{else}>
            <p class="state" style="color: orangered;">
                <span>您尚未配置腾讯云信息</span>
        ...
        <div>
            <a href="/wxapp/currency/tencentCfg" class="btn btn-sm btn-green"><i class="icon-plus bigger-80"></i> 腾讯云配置</a>
                    ...
                    <table id="sample-table-1" class="table table-striped table-hover table-avatar">
                        ...
                        <{foreach $list as $val}>
                            <tr>
                                <td><{$val['aqv_title']}></td>
                                <td><{$val['aqv_type']}></td>
                                <td><{$val['aqv_video_url']}></td>
                                <td><{$val['aqv_time']}>s</td>
                                <td><{date('Y-m-d H:i',$val['aqv_update_time'])}></td>
                                <td>
                                    <a href="#" data-id="<{$val['aqv_id']}>" data-title="<{$val['aqv_title']}>" data-url="<{$val['aqv_video_url']}>" onclick="editVideo(this)" class="btn btn-xs btn-green">重传</a>
                                    <a href="#" data-clipboard-text="<{$val['aqv_video_url']}>" class="copy_input btn btn-xs btn-green">复制链接</a>
                                    <a href="<{$val['aqv_video_url']}>?v=<{$val['aqv_time']}>" target="_blank" class="btn btn-xs btn-green">预览</a>
                                    <a href="#" id="delete-confirm"  onclick="deleteVideo('<{$val['aqv_id']}>')" class="btn btn-xs btn-danger del-btn">删除</a>
                                </td>
                            </tr>
                            <{/foreach}>
...
<script>
    ...
    function getUplode(){
        ...
        $.ajax({
            'type'  : 'post',
            'url'   : '/wxapp/currency/uploadTencentCfg',
            'data'  : {type: fileType, key: key},
            'dataType'  : 'json',
            'success'   : function(ret){
                layer.close(index);
                if(ret.ec == 200){
                    putObject(ret.data.key);
                }else{
                    layer.msg(ret.em);
                }
            }
