...
<div class="balance clearfix" style="border:1px solid #e5e5e5;margin-bottom: 20px;">
	<div class="balance-info">
		...
		<div class="balance-content">
			<span class="money"><{if $todayTradeInfo['money'] ||  $todayTradeInfo['coinMoney']}><{$todayTradeInfo['money'] + $todayTradeInfo['coinMoney']}><{else}>0<{/if}></span>
	...
	<div class="balance-info">
		...
		<div class="balance-content">
			<span class="money money-font"><{if $searchTradeInfo['money'] || $searchTradeInfo['coinMoney']}><{$searchTradeInfo['money'] + $searchTradeInfo['coinMoney']}><{else}>0<{/if}></span>
			<span class="unit">元</span>
		...
		<form class="form-inline" action="/wxapp/order/tradeList" method="get">
			<div class="col-xs-11 form-group-box">
				<div class="form-container" style="width: auto !important;">
					...
					<div class="form-group" <{if $appletCfg['ac_type'] == 4}>style="display:none"<{/if}>>
						<div class="input-group">
...
<div class="choose-state">
	...
	<a href="/wxapp/order/tradeList?status=<{$key}>" <{if $status && $status eq $key}>class="active"<{/if}>><{$val['label']}></a>
	<{/foreach}>
	<!---
	...
	<table class="ui-table-order" style="padding: 0px;">
			...
			<td colspan="7">
				<div>
					...
					<div class="help" style="display: inline-block;">
						...
						<{if $val['t_status'] == 3 && $val['t_remind_time']>0}>
						<span style="color: red;padding-left: 20px;">
							<{date("m-d H:i", $val['t_remind_time'])}>提醒发货
						</span>
						<{/if}>
					</div>
				</div>
		...
		<tr class="content-row">
			...
			<td class="format-cell">
				<{$trader[$val['t_id']]['data'][0]['to_gf_name']}>
			</td>
			<td class="price-cell">
				<p>
					<{$val['t_total_fee']}>
			...
			<td class="time-cell" rowspan="1">
				...
				<div class="td-cont">
					<p class="user-name" style="margin-top: 10px">
						<a href="/wxapp/order/tradeList?postType=<{$val['t_express_method']}>" class="new-window" target="_blank">
						<{$expressMethod[$val['t_express_method']]}>
						</a>
					</p>
				</div>
				<{if $val['t_express_method'] == 5 && $val['t_status'] == 3}>
				<!--<p class="user-name">
				...
				<div class="td-cont">
					<p class="user-name" style=";margin-top: 10px">
						<{$storeSelect[$val['t_store_id']]['os_name']}>
					</p>
				</div>
				<{/if}>
			</td>
			<td class="state-cell" rowspan="1">
				<div class="td-cont">
					<p class="js-order-state" id="status_<{$val['t_tid']}>">
						<{if $val['t_pay_type'] == 4 && $val['t_status'] == 3 && $appletCfg['ac_type'] == 21}>
						已下单
						<{else}>
					...
					<{if $val['t_status'] == 6}>
					<!-- 订单完成 -->
					<{if $val['t_finish_type'] == 0 && $val['t_manager_name']}>
					<p>
						<span>由<span style="color: blue"><{$val['t_manager_name']}></span>操作</span>
					</p>
					<{elseif $val['t_finish_type'] == 1 && $val['t_manager_name']}>
					<p>
						<span>由店铺<span style="color: blue"><{$val['t_manager_name']}></span>操作</span>
					</p>
					<{elseif $val['t_finish_type'] == 2}>
					<p>
						<span>自动完成</span>
					</p>
					<{/if}>
			...
			<td class="pay-price-cell" rowspan="1">
				<div class="td-cont text-center">
					<div>
						...
						<p class="user-name">
							<{if $val['t_status'] == 3 && ($val['t_express_method'] == 1 || $val['t_express_method'] == 3)}>
						...
						<span id="refund_<{$val['t_tid']}>" class="btn btn-primary btn-xs refund-btn-modal" data-toggle="modal" data-target="#refundNewModal"
							  data-tid="<{$val['t_tid']}>"
						...
						<{if $val['t_status'] == 3 || $val['t_status'] == 4 || ($val['t_status'] == 5 && $val['t_legwork_extra'])}>
						<p class="user-name">
		...
		<tr class="remark-row buyer-msg">
			...
			<td colspan="1" style="text-align:center;">
				<a href="javascript:;" data-note='<{$val['t_note']}>' data-tid="<{$val['t_tid']}>" class="edit-trade-note">编辑</a>
			</td>
		</tr>
		<{/if}>
		...
		>
		    ...
		    <td style="text-align: center">
				<a href="javascript:;" data-remark='<{json_encode($val['t_remark_extra'])}>' data-tid="<{$val['t_tid']}>" class="edit-trade-remark">编辑</a>
			</td>
		</tr>
		<{/if}>
		<{if !$val['t_note'] && (count($val['t_remark_extra']) == 1 && $val['t_remark_extra'][0]['name']=='备注' && !$val['t_remark_extra'][0]['value'])}>
			...
			<div class="modal-body" style="overflow: auto;text-align: center;margin-bottom: 45px">
				<div class="modal-plan p_num clearfix shouhuo">
					<form id='trade-export-form' enctype="multipart/form-data" action="/wxapp/order/excelOrderNew" method="post" onsubmit="return false">
						<input type="hidden" name="independent" value="<{$independent}>">
						<div class="form-group">
