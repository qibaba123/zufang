        ...
        <div class="content-region clearfix">
            <div class="info-region" style="padding: 15px;">
                ...
                <{if $refund_order_list}>
                    <!-- 单品退款 -->
                    <{foreach $refund_order_list as $item}>
                        <div>
                            <div class="info-goods">
                                <div class="ui-centered-image" src="<{$item['t_pic']}>"><img src="<{$item['t_pic']}>"></div>
                                <div class="info-goods-content">
                                    <div><{$item['t_title']}></div>
                                    <div></div>
                                </div>
                            </div>
                        ...
                        <table class="info-table">
                            <tbody>
                                ...
                                <tr>
                                    <th>退款金额：</th>
                                    <td><span class="color-orange"><{$item['tr_money']}></span>
                                        元
                                        <{if $row['t_type'] != 6}>
        	...
        	<div class="state-region">
        	    <div style="padding: 0px 0px 30px 40px;">
        	        <h3 class="state-title">
                        ...
                        <{elseif $row['t_feedback'] eq 1 && $row['t_fd_status'] eq 4}>
                    ...
                    <{if $refund_order_list==null}>
                        <{if $row['t_fd_status'] eq 1 || $row['t_fd_status'] eq 4}>
                        ...
                        <{foreach $refund_order_list as $item}>
                            <div class="state-action">
                                <div class='refund-img'>
                                    <img style="width: 60px;height: 60px;" src="<{$item['t_pic']}>">
                                </div>
                                <{if $item.canAgree && $item['to_fd_status'] eq 1}>
                                <div class="deal_refund ui-btn ui-btn-<{if $alert['errno'] < 0}>default<{else}>primary<{/if}>" data-type='agreeTK'  data-money='<{$item.tr_money}>' data-order='<{$item.to_id}>' data-toggle="modal" >同意买家退款</div>
                                <{/if}>
                                <{if $item.canRefuse && $item['to_fd_status'] eq 1}>
                                <div class="deal_refund ui-btn"  data-order='<{$item.to_id}>' data-money='<{$item.tr_money}>' data-type='refuseTK' data-toggle="modal" >拒绝退款申请</div>
                                <{/if}>

                                <!-- 处理结果 -->
                                <{if $item['to_fd_status'] eq 1 || $item['to_fd_status'] eq 4}>
                                    <div class="state-desc" style='display: inline-block;margin-left: 10px;'>
                                        <div>
                                            收到买家仅退款申请，请尽快处理。
                                            <p>请在<span class="color-orange"><em id='remain-time-<{$item.to_id}>'  class="remain-time" data-time='<{$item.endTime}>' data-order='<{$item.to_id}>'></em></span>处理本次退款，如逾期未处理，将自动同意退款。</p>
                                        </div>
                                    </div>
                                    <{elseif $item['to_fd_status'] eq 2}>
                                    <div class="state-desc">
                                        <div>
                                            你已拒绝买家的退款申请。
                                            <p>如果买家在<span class="color-orange"><em id='remain-time-<{$item.to_id}>' class="remain-time"  data-time='<{$item.endTime}>' data-order='<{$item.to_id}>'></em></span>未继续申请退款，维权将自动结束。</p>
                                        </div>
                                    </div>
                                    <{else}>
                                    <div class="state-desc">
                 ...
                 <div class="modal-body">
                    ...
                    <div class="form-horizontal" style="margin: 0;">
                        ...
                        <div class="control-group">
                            ...
                            <div class="controls">
                                <div class="control-action refund_money" style="color: #f60;">¥ <{$refund['tr_money']}></div>
                            </div>
                        </div>
                    </div>
                 </div>
