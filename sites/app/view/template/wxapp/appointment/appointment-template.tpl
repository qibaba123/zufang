    ...
    <div class="edit-right">
        <div class="edit-con">
            ...
            <div class="address" data-right-edit data-id="4">
                <div class="input-group-box" <{if $appletCfg['ac_type'] != 6}>style="display: none" <{/if}>>
...
<script>
    ...
    app.controller('chCtrl',['$scope','$http','$timeout', function($scope,$http,$timeout){
        ...
        $scope.headerTitle = '<{$tpl['aai_title']}>'?'<{$tpl['aai_title']}>':"预约";
        $scope.openTime    = '<{$tpl['aai_open_time']}>';
        $scope.mobile      = '<{$tpl['aai_mobile']}>';
        $scope.address     = '<{$tpl['aai_address']}>';
        $scope.showAddress = '<{$tpl['aai_show_address']}>' > 0 ?true:false;
        $scope.goodTitle   = '<{$tpl['aai_good_title']}>'?'<{$tpl['aai_good_title']}>':"预约项目";
        $scope.orderTitle  = '<{$tpl['aai_order_title']}>'?'<{$tpl['aai_order_title']}>':"预约订单";
        $scope.lng         = '<{$tpl['aai_lng']}>'? '<{$tpl['aai_lng']}>' : '113.72052';
        $scope.lat         = '<{$tpl['aai_lat']}>'? '<{$tpl['aai_lat']}>' : '34.77485';
        $scope.noticeTxt   = <{$noticeList}>;
        $scope.articles    = <{$articles}>;
        $scope.banners     = <{$slide}>;
        $scope.mustTime    = <{$tpl['aai_musttime']}> ? true : false ;
        $scope.mustAddress = <{$tpl['aai_mustaddress']}> ? true : false ;
        /*新增*/
        $scope.appointTitle = '<{$tpl['aai_appoint_title']}>'?'<{$tpl['aai_appoint_title']}>':"预约简介";
        $scope.buttonText = '<{$tpl['aai_button_text']}>'?'<{$tpl['aai_button_text']}>':"预约";
