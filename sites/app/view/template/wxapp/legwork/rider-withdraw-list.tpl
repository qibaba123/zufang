...
<div  id="mainContent">
    ...
    <div class="page-header" style="overflow:hidden">
        ...
        -->
        <div class="col-sm-1">
            <a class="btn btn-green btn-sm" href="/wxapp/legwork/riderWithdrawCfg">
    ...
    <div ng-app="Withdraw"  ng-controller="WithdrawList">
        <div class="page-header search-box">
            <div class="col-sm-12">
                <form class="form-inline" action="/wxapp/legwork/riderWithdraw">
        ...
        <div class="choose-state">
            <a href="/wxapp/legwork/riderWithdraw" <{if !$audit}> class="active" <{/if}>>全部提现申请</a>
            <a href="/wxapp/legwork/riderWithdraw?audit=audit" <{if $audit eq 'audit'}> class="active" <{/if}>>待审核</a>
            <a href="/wxapp/legwork/riderWithdraw?audit=pass" <{if $audit eq 'pass'}> class="active" <{/if}>>已通过</a>
            <a href="/wxapp/legwork/riderWithdraw?audit=refuse" <{if $audit eq 'refuse'}> class="active" <{/if}>>已拒绝</a>
        </div>
        <div class="row">
            <div class="col-xs-12">
                <div class="fixed-table-box" style="margin-bottom: 30px;">
                    ...
                    <div class="fixed-table-body">
                        <table id="sample-table-1" class="table table-hover">
                            ...
                            <{foreach $list as $val}>
                                <tr>
                                    <td><a href="/wxapp/legwork/riderWithdraw?name=<{$val['alr_name']}>"><{$val['alr_name']}></a></td>
                                    <td><{$val['alr_mobile']}></td>
                                    <td>
                                        <{if $val['alrw_type'] == 1}>
                                            佣金提现
                                        <{else}>
                                    ...
                                    <td style="text-align: left">
                                        <p>
                                            方式：<{if isset($withdrawType[$val['alrw_way']])}><{$withdrawType[$val['alrw_way']]}><{/if}>
                                        </p>
                                        <p>
                                            名称：<{if $val['alrw_bank']}><{$banks[$val['alrw_bank']]}>&nbsp;<{/if}><{$val['alrw_name']}>
                                        </p>
                                        <{if $val['alrw_bank_user']}>
                                        <p>
                                            开户人：<{$val['alrw_bank_user']}>
                                        </p>
                                        <{/if}>
                                        <p>
                                            账号：<{$val['alrw_account']}>
                                    ...
                                    <td style="text-align: left">
                                        <p>
                                            申请金额：<{floatval($val['alrw_money'])}>
                                        </p>
                                        <{if $val['alrw_deduct']>0}>
                                        <p>
                                            平台抽成：<{floatval($val['alrw_deduct'])}>
                                        </p>
                                        <{/if}>
                                        <p>
                                            实际金额：<span style="color: blue"><{$val['alrw_money'] - $val['alrw_deduct']}></span>
                                        </p>
                                    </td>
                                    <td>
                                        <!--
                                        <span class="label label-sm label-<{$withdraw_status[$val['alrw_status']]['css']}>"><{$withdraw_status[$val['alrw_status']]['label']}></span>
                                        -->
                                        <span class="<{$withdraw_status_new[$val['alrw_status']]['class']}>"><{$withdraw_status_new[$val['alrw_status']]['label']}></span>
                                    </td>
                                    <td><{date('y-m-d H:i:s',$val['alrw_create_time'])}></td>
                                    <td>
                                        <{$val['alrw_handle_note']}>
                                    </td>
                                    <td>
                                        <{if $val['alrw_status'] eq 1}>
...
//            }
        ...
        $('.modal-save').on('click',function(){
            ...
            if(id && status){
                ...
                $.ajax({
                    'type'  : 'post',
                    'url'   : '/wxapp/legwork/dealWithdraw',
                    'data'  : data,
                    'dataType'  : 'json',
