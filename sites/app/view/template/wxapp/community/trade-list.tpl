...
<div class="balance clearfix" style="border:1px solid #e5e5e5;margin-bottom: 20px;">
		...
		<div class="balance-info">
			...
			<div class="balance-content">
				<span class="money"><{if $todayTradeInfo['points']}><{$todayTradeInfo['points']}><{else}>0<{/if}></span>
		...
		<div class="balance-info">
			...
			<div class="balance-content">
				<span class="money money-font"><{if $searchTradeInfo['points']}><{$searchTradeInfo['points']}><{else}>0<{/if}></span>
				<span class="unit">积分</span>
			</div>
		</div>
        <div class="balance-info">
	...
	<div class="choose-state">
		...
		<a href="<{$searchUrl}>?status=<{$key}><{if $esId}>&esId=<{$esId}><{/if}>" <{if $status && $status eq $key}>class="active"<{/if}>><{$val['label']}></a>
		<{/foreach}>
		<!---
	...
	<div class="trade-list">
		<table class="ui-table-order" style="padding: 0px;">
			...
			<tr class="content-row">
				...
				<td class="title-cell">
					<p class="goods-title">
						<a href="/<{$searchUrl}>?title=<{$val['t_title']}>"class="new-window" title="<{$val['t_title']}>">
							<{$val['t_title']}>
						</a>
					</p>
					<p>
				...
				<td class="price-cell">
					<p>
						<{$val['t_total_fee']}><{if $val['t_applet_type'] == 4}> + 积分<{$val['t_points']}><{/if}>
					</p>
					<p>(<{$val['t_num']}>件)</p>
				</td>
				<td class="aftermarket-cell" rowspan="1">
					...
					<{if $pointOrder == 1}>
					<a href="/wxapp/community/pointTradeRefund?order_no=<{$val['t_tid']}><{if $esId}>&esId=<{$esId}><{/if}>" class="new-window" >处理维权</a>
					<{else}>
					<a href="/wxapp/community/tradeRefund?order_no=<{$val['t_tid']}><{if $esId}>&esId=<{$esId}><{/if}>" class="new-window" >处理维权</a>
					<{/if}>
					<{/if}>
				</td>
				<td class="customer-cell" rowspan="1">
					<p>
						<a href="<{$searchUrl}>?buyer=<{$val['t_buyer_nick']}>" class="new-window" target="_blank">
							<{$val['t_buyer_nick']}>
						</a>
					</p>
				...
				<td class="time-cell" rowspan="1">
					...
					<div class="td-cont">
						<p class="user-name" style="margin-top: 10px">
							<a href="/wxapp/community/pointOrder?postType=<{$val['t_express_method']}>" class="new-window" target="_blank">
