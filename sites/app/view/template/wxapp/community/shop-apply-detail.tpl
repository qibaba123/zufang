				...
				<div class="info-group-inner">
					<div class="group-info" style="background-color: #fff">
						<div class="payment-block-body js-wxpay-body-region" style="display: block;">
							<div>
								<form action="">
									<input type="hidden" class="form-control" id="acsId" value="<{if $row}><{$row['es_id']}><{/if}>" >
							<{if $row['es_add_from'] == 4}>
									<div class="form-group">
										<label for="firstname" class="col-sm-2 control-label text-right">店铺类型：</label>
										<div class="col-sm-3">
											<{if $row['es_shop_type'] == 2}>
											企业
											<{else}>
											个人
											<{/if}>
							...
							<div class="form-group">
								...
								<div class="col-sm-3">
									<{if $row && $row['es_name']}><{$row['es_name']}><{/if}>
								</div>
							</div>
									<div class="form-group">
										<label for="firstname" class="col-sm-2 control-label text-right">申请电话：</label>
										<div class="col-sm-3">
											<{if $row && $row['es_phone']}><{$row['es_phone']}><{/if}>
										</div>
									</div>
									<div class="form-group">
										<label for="firstname" class="col-sm-2 control-label text-right">负责人：</label>
										<div class="col-sm-3">
											<{if $row && $row['es_contact']}><{$row['es_contact']}><{/if}>
										</div>
									</div>
									<div class="form-group">
										<label for="firstname" class="col-sm-2 control-label text-right">服务类别：</label>
										<div class="col-sm-3">
											<{$category[$row['es_cate1']]}> - <{$category[$row['es_cate2']]}>
										</div>
									</div>
									<{if $shop['es_add_from'] == 4}>

									<div class="form-group">
										<label for="firstname" class="col-sm-2 control-label text-right">入驻时长：</label>
										<div class="col-sm-3">
											<{$row['daysValue']}>
										</div>
									</div>
									...
									<div class="form-group">
										<label for="firstname" class="col-sm-2 control-label text-right">所在地区：</label>
										<div class="col-sm-3">
											<{$row['prov_name']}>/<{$row['city_name']}>/<{$row['zone_name']}>
										</div>
									</div>
									...
									<div class="form-group">
										<label for="firstname" class="col-sm-2 control-label text-right">所在商圈：</label>
										<div class="col-sm-3">
											<{$district[$row['es_district2']]['area_name']}> - <{$district[$row['es_district2']]['name']}>
										</div>
									</div>
                  ...
                  <{if $menuType == 'toutiao'}>
									<div class="form-group">
										<label for="firstname" class="col-sm-2 control-label text-right">所在区域：</label>
										<div class="col-sm-3">
											<{$row['es_prov_name']}>-<{$row['es_city_name']}>-<{$row['es_zone_name']}>
										</div>
									</div>
									<{/if}>
									...
									<div class="form-group">
										<label for="firstname" class="col-sm-2 control-label text-right"><{if $row['es_add_from'] == 4 && $row['es_shop_type'] == 1}>手持身份证照片：<{else}>营业执照：<{/if}></label>
										<div class="col-sm-3" style="width: 80%">
											<{if $row['es_license']}>
											<img src="<{$row['es_license']}>" alt="营业执照">
											<{/if}>
										</div>
									</div>

									<{if $row['es_add_from'] == 4 && $row['es_shop_type'] == 2}>
									<div class="form-group">
										<label for="" class="col-sm-2 control-label text-right">身份证正面</label>
										<div class="col-sm-3" style="width: 80%">
											<{if $row['es_card_front']}>
										<img src="<{$row['es_card_front']}>" alt="营业执照">
											<{/if}>
										</div>
									</div>
									<div class="form-group">
										<label for="" class="col-sm-2 control-label text-right">身份证反面</label>
										<div class="col-sm-3" style="width: 80%">
											<{if $row['es_card_back']}>
										<img src="<{$row['es_card_back']}>" alt="营业执照">
											<{/if}>
										</div>
									</div>
									<{/if}>
									...
									<div class="form-group">
										...
										<div class="col-sm-3">
											<{if $row['es_handle_status'] == 2}>
											<span style="color: green">已通过</span>
											<{elseif $row['es_handle_status'] == 3}>
...
<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	<div class="modal-dialog" style="width: 535px;">
		<div class="modal-content">
			<input type="hidden" id="hid_id" value="<{$row['es_id']}>">
