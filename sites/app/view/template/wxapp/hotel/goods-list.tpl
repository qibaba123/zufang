...
<div  id="content-con" >
    <a href="/wxapp/hotel/addGood" class="btn btn-green btn-xs"><i class="icon-plus bigger-80"></i> 新增</a>
    <div class="page-header search-box">
        <div class="col-sm-12">
            <form action="/wxapp/hotel/goods" method="get" class="form-inline">
                <div class="col-xs-11 form-group-box">
                    <div class="form-container">
                        ...
                        <div class="form-group">
                            <div class="input-group">
                                <div class="input-group-addon" style="height:34px;">所属门店</div>
                                <select id="store" name="store" style="height:34px;width:100%" class="form-control">
                                    <option value="0">全部</option>
                                    <{foreach $storeList as $key => $val}>
                                    <option <{if $store eq $val['ahs_id']}>selected<{/if}> value="<{$val['ahs_id']}>"><{$val['ahs_name']}></option>
                                    <{/foreach}>
                                </select>
                            </div>
                        </div>
                    ...
                    <table id="sample-table-1" class="table table-striped table-hover table-avatar">
                        ...
                        <{foreach $list as $val}>
                            <tr id="tr_<{$val['g_id']}>">
                                ...
                                <td>
                                    <{if $storeList[$val['g_kind1']]}>
                                    <{$storeList[$val['g_kind1']]['ahs_name']}>
                                    <{/if}>
                                </td>
                                <td><{$val['g_bed_info']}></td>
                                <td><{if $val['g_has_window'] == 1}>是<{else}>否<{/if}></td>
                                <td><{if $val['g_add_bed'] == 1}>是<{else}>否<{/if}></td>
                                <td><{$val['g_room_size']}></td>
                                <td><{date('Y-m-d H:i:s',$val['g_update_time'])}></td>

                                <td>
                                    <a href="/wxapp/hotel/addGood/?id=<{$val['g_id']}>" >编辑</a> -
