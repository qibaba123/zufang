<link rel="stylesheet" href="/public/plugin/emoji/lib/css/jquery.mCustomScrollbar.min.css"/>
...
<div class="reply" ng-app="chApp" ng-controller="chCtrl">
    ...
    <div style="margin-left:370px;padding-top:1px;">
        ...
        <div class="auto-reply">
            ...
            <div class="reply-footer" ng-show="msgType=='text'" >
                ...
                <div class="insert-text">
                    ...
                    <{if $type != $val['type']}><BR/><{/if}>
                    <span class="insert-txt <{$type=$val['type']}>" data-type="<{$val['type']}>" data-content='<{$val['content']}>'><{$val['label']}></span>
                    <{/foreach}>
                    <br/>
        ...
        <div class="btn-box">
            <input type="hidden" name="type" id="type" value="<{$msg['wm_type']}>">
...
<script>
    ...
    app.controller('chCtrl', ['$scope','$http','$timeout',function($scope, $http, $timeout){
        $scope.msgType    = '<{$msg['asm_type']}>'?'<{$msg['asm_type']}>':'text';
        $scope.msgKeyword = '<{$msg['asm_keyword']}>'?'<{$msg['asm_keyword']}>':'';
        $scope.msgContent = `<{$msg['asm_content']}>`?`<{$msg['asm_content']}>`:'回复内容';
        $scope.msgTitle   = '<{$msg['asm_title']}>'?'<{$msg['asm_title']}>':'回复标题';
        $scope.msgDesc    = `<{$msg['asm_desc']}>`?`<{$msg['asm_desc']}>`:'回复描述';
        $scope.msgPath    = '<{$msg['asm_path']}>'?'<{$msg['asm_path']}>':'';
        $scope.msgUrl     = '<{$msg['asm_url']}>'?'<{$msg['asm_url']}>':'';
        $scope.msgCover   = '<{$msg['asm_cover']}>'?'<{$msg['asm_cover']}>':'/public/manage/img/zhanwei/zw_fxb_200_200.png';
        $scope.appid      = '<{$appletCfg['ac_appid']}>';
        $scope.appCover   = '<{$appletCfg['ac_avatar']}>';
        $scope.appName    = '<{$appletCfg['ac_name']}>';
