        ...
        <li <{if $val['it_id'] == $row['it_id']}>class="usingtem" <{/if}>>
            <div class="temp-img">
                ...
                <div class="use-edit">
                    ...
                    <a href="/wxapp/train/indexTpl?tpl=<{$val['it_id']}>" class="btn btn-xs btn-primary">编辑</a>
                </div>
            </div>
            <p><{$val['it_name']}></p>
            </li>
            <{/foreach}>
            <li <{if $appletCfg['ac_index_tpl'] == 0}> class="usingtem" <{/if}>>
...
<script>
    ...
    $('.btn-start').on('click',function(){
        ...
        $.ajax({
            'type'  : 'post',
            'url'   : '/wxapp/train/startAppletTpl',
            'data'  : data,
            'dataType' : 'json',
