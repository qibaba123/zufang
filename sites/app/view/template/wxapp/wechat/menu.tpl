...
<div ng-app="youzan" ng-controller="menuCtrl" class="custommenu-box" id="custommenu-box" style="position:static;">
    <div class="page-con" style="margin-bottom: 70px;">
        ...
        <div class="alert alert-warning setting-save" role="alert"><button class="btn btn-success btn-sm" ng-click="synTowx();">同步微信</button> &nbsp; &nbsp; <button class="btn btn-primary btn-sm" ng-click="saveMenu();">提交修改</button></div>
    </div>
    <!-- 确定删除 -->
...
<script>
    var Youzan=angular.module("youzan",["ui.sortable","RootModule"]);
    ...
    Youzan.controller("menuCtrl",["$scope","$timeout", '$http', function($scope,$timeout,$http){
        ...
        $scope.addNewFirstMenu=function(){
            ...
            if(menu_length<3){
                ...
                //保存一级菜单到数据库
                //整体保存 saveMenu($scope.defaultText,-1,menu_length);
                var menufirst={
                    // index:menu_length,
                    id:0,
                    index:defaultIndex,
        ...
        $scope.delMenu=function(){
            ...
            //整体保存 delMenu(parentIndex,sonIndex);
        ...
        $scope.editMenu=function(){
            ...
            if(parentIndex<0){
                //完成编辑
                var str_length = getStringLength($scope.editMenuText);   // 获取菜单长度
                if(str_length<=8){
                    ...
                    //保存菜单修改
                    //整体保存 saveMenu($scope.editMenuText,parentIndex,sonIndex);
                }else{
                    $scope.tiptext="一级菜单最多不能超过4个汉字或者8个字母";
                    $scope.statusClass="bg1";
                    $scope.noticetip=true;
                    $timeout(function () {
                        $scope.noticetip=false;
                ...
                if(str_length<=16){
                    ...
                    $scope.tiptext="二级菜单最多不能超过8个汉字或者16个字母";
                    $scope.statusClass="bg1";
                    $scope.noticetip=true;
                    $timeout(function () {
                        $scope.noticetip=false;
