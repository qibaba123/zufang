                ...
                <div class="panel panel-default" style="width: 550px;<{if $manager['m_fid'] > 0}>display:none;<{/if}>" >
                    ...
                    <div class="panel-body">
                        <form class="form-horizontal" role="form">
                            <input type="hidden" id="hid_id"  class="avatar-field bg-img" name="hid_key" value="<{if $blessingCfg && $blessingCfg['abc_id']}><{$notice['abc_id']}><{/if}>"/>
                            <div class="form-group">
                                <label class="col-sm-2 control-label no-padding-right" for="yuan-pass"> 祝福标题：</label>
                                <div class="col-sm-10">
                                    <input type="text" class="form-control col-sm-8" id="blessingTitle" value="<{if $blessingCfg && $blessingCfg['abc_blessing_title']}><{$blessingCfg['abc_blessing_title']}><{/if}>" placeholder="请输入祝福标题">
                                </div>
                            </div>
                            <div class="form-group">
                                <label class="col-sm-2 control-label no-padding-right" for="new-pass"> 祝福内容： </label>
                                <div class="col-sm-10">
                                    <textarea type="text" class="form-control col-sm-8" rows="8" id="blessingContent" placeholder="请输入默认祝福内容"><{if $blessingCfg && $blessingCfg['abc_blessing']}><{$blessingCfg['abc_blessing']}><{/if}></textarea>
                                </div>
                            </div>
                            <div class="form-group">
