...
<div>
	...
	<a href="javascript:;" class="btn btn-blue btn-xs btn-excel-activity" ><i class="icon-download"></i>活动订单导出</a>
	<{/if}>
	<a href="javascript:;" class="btn btn-green btn-xs btn-print-list" ><i class="icon-print"></i>订单打印</a>
</div>
<div class="page-header search-box">
	<div class="col-sm-12">
		<form class="form-inline" action="/wxapp/sequence/tradeList" method="get">
...
<div class="choose-state">
	...
	<a href="/wxapp/sequence/tradeList?status=<{$key}>" <{if $status && $status eq $key}>class="active"<{/if}>><{$val['label']}></a>
	<{/foreach}>
	<!---
	...
	<table class="ui-table-order" style="padding: 0px;">
		<thead class="js-list-header-region tableFloatingHeaderOriginal" style="position: static; top: 0px; margin-top: 0px; left: 225px; z-index: 1; width: 794px;">
		    <tr class="widget-list-header">
		        ...
		        <th class="time-cell" style="min-width: 100px; max-width: 100px;">订单状态</th>
		...
		<tr class="content-row">
			...
			<td class="title-cell">
				...
				<p class="goods-title">
					活动名称：
					<a href="/wxapp/sequence/tradeList?activity=<{$val['asa_title']}>"class="new-window" title="<{$val['asa_title']}>">
						<{$val['asa_title']}>
					</a>
				</p>
				-->
				...
				-->
				<p>
					小区：<a href="/wxapp/sequence/tradeList?community=<{$val['asc_name']}>"class="new-window" title="<{$val['asc_name']}>">
						<{$val['asc_name']}>
					</a>
				</p>
				<p>
					团长：<a href="/wxapp/sequence/tradeList?leader=<{$val['asl_name']}>"class="new-window" title="<{$val['asl_name']}>">
						<{$val['asl_name']}>
					</a>
				</p>
				<!--
					<{if $val['t_se_send_time']}>
					<p>
						自提/配送时间：<{date('Y-m-d',$val['t_se_send_time'])}>
					</p>
					<{/if}>
					-->
				<{if $val['t_se_num'] > 0}>
			...
			<td class="customer-cell" rowspan="1">
				<p>
					<a href="/wxapp/sequence/tradeList?buyer=<{$val['t_buyer_nick']}>" class="new-window" target="_blank">
						<{$val['t_buyer_nick']}>
					</a>
				</p>
			...
			<td class="time-cell" rowspan="1">
				...
				<div class="td-cont">
					<p class="user-name" style="margin-top: 10px">
						<a href="/wxapp/sequence/tradeList?postType=<{$val['t_express_method']}>" class="new-window" target="_blank">
			...
			<td class="pay-price-cell" rowspan="1">
				<div class="td-cont text-center">
					<div>
						...
						<p class="user-name">
							<{if ($val['t_status'] == 2 || $val['t_status'] == 3 || $val['t_status'] == 4) && $val['t_fd_status'] != 3}>
		...
		<div class="modal-content">
			...
			<div class="modal-body" style="overflow: auto;text-align: center;margin-bottom: 45px">
				<div class="modal-plan p_num clearfix shouhuo">
					<form enctype="multipart/form-data" action="/wxapp/sequence/excelOrder" method="post">
						<div class="form-group">
						...
						<div class="form-group">
							...
							<div class="col-sm-10">
								<select id="communityId" name="communityId" class="form-control" style="width: 37%;">
									<option value="0">全部</option>
				...
				<div class="modal-plan p_num clearfix shouhuo">
					<form>
						...
						<div class="form-group">
							<label class="col-sm-2 control-label">开始日期</label>
							<div class="col-sm-4">
								<input class="form-control date-picker" type="text" id="print_startDate" data-date-format="yyyy-mm-dd" name="print_startDate" placeholder="请输入开始日期"/>
							</div>
							<label class="col-sm-2 control-label">开始时间</label>
							<div class="col-sm-4 bootstrap-timepicker">
						...
						<div class="form-group">
							<label class="col-sm-2 control-label">结束日期</label>
							<div class="col-sm-4">
								<input class="form-control date-picker" type="text" id="print_endDate" data-date-format="yyyy-mm-dd" name="print_endDate" placeholder="请输入结束日期"/>
							</div>
							<label class="col-sm-2 control-label">结束时间</label>
							<div class="col-sm-4 bootstrap-timepicker">
