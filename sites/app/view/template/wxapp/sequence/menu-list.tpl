                        ...
                        <div class="form-group">
                            <div class="input-group">
                                ...
                                <select id="cate" name="cate" style="height:34px;width:100%" class="form-control my-select2">
                                    ...
                                    <option <{if $cate eq $val['id']}>selected<{/if}> value="<{$val['id']}>"><{$val['title']}></option>
                                    <{/foreach}>
                                </select>
                            </div>
                        </div>
            ...
            <div class="fixed-table-box" style="margin-bottom: 30px;">
                <div class="fixed-table-body">
                    <table id="sample-table-1" class="table table-hover table-avatar">
                        ...
                        <{foreach $list as $val}>
                            <tr id="tr_<{$val['ams_id']}>" class="tr-content" style="border-bottom:0">
                                <!-- 选择框 -->
                                ...
                                <!-- 排序 -->
                                <td style="width:40px;">
                                    <span class='span'><{$val['asm_sort']}></span>
                                    <img src="/public/wxapp/images/icon_edit.png" class="good-admend set-goodsinfo" data-id="<{$val['asm_id']}>" data-value="<{$val['asm_sort']}>" data-field="sort" />
                                </td>
                                <!-- 商品 -->
                                <td class="proimg-name" style="min-width: 270px; width: 320px;">
                                    <{if isset($val['asm_cover'])}>
                                    <img src="<{$val['asm_cover']}>" width="75px" height="75px" alt="封面图" style="border-radius:4px;">
                                    <{/if}>
                                    <div>
                                        <p class="pro-name" style="margin-bottom:6px;">
                                            <a href="/wxapp/sequence/editMenu?id=<{$val['asm_id']}>">
                                                <{if mb_strlen($val['asm_title']) > 20 }><{mb_substr($val['asm_title'],0,20)}>
                                                <{mb_substr($val['asm_title'],20,40)}><{else}><{$val['asm_title']}><{/if}>
                                            </a>
                                        </p>
                                        <{if $menuCate[$val['asm_category']]}>
                                        <p style='font-weight: bold;color:#666;'>
                                            分类&nbsp;[<span style='color: #9a999e;'><{$menuCate[$val['asm_category']]['title']}></span>]
                                        </p>
                                        <{/if}>
                                    </div>
                                ...
                                <td >
                                    <p id="share_num_<{$val['asm_id']}>">
                                        转发数量：<span class='span'><{$val['asm_share_num']}></span>
                                        <img src="/public/wxapp/images/icon_edit.png" class="good-admend set-goodsinfo" data-id="<{$val['asm_id']}>" data-value="<{$val['asm_share_num']}>" data-field="share_num" />
                                    </p>
                                    <p id="like_num_<{$val['asm_id']}>">
                                       点赞数量： <span class='span'><{$val['asm_like_num']}></span>
                                        <img src="/public/wxapp/images/icon_edit.png" class="good-admend set-goodsinfo" data-id="<{$val['asm_id']}>" data-value="<{$val['asm_like_num']}>" data-field="like_num" />
                                    </p>
                                </td>
                                <td>
                                    <{date('Y-m-d H:i',$val['asm_create_time'])}>
                                </td>
                                <!-- 操作 -->
                                <td class='new-action' style="color:#ccc;">
                                    <p>
                                        <!-- 区域管理合伙人隐藏此选项 -->
                                        <a href="/wxapp/sequence/editMenu/?id=<{$val['asm_id']}>" title='编辑' >编辑</a>
                                        <a href="javascript:;" id="del_<{$val['asm_id']}>" class="btn-del" data-gid="<{$val['asm_id']}>" title='删除' >删除</a>
                                    </p>
                                </td>
                            </tr>
                        <{/foreach}>
