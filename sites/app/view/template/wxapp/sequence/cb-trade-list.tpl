...
<div class="page-header search-box">
	<div class="col-sm-12">
		<form class="form-inline" action="<{if $limit_trade == 1}>/wxapp/limit/sequenceOrder<{elseif $bargain_trade == 1}>/wxapp/bargain/sequenceOrder<{else}>/wxapp/sequence/tradeList<{/if}>" method="get">
					...
					<div class='row'>
						...
						<div class="form-group" style="width: 520px">
							<div class="input-group">
								...
								<input type="text" class="form-control" name="start" value="<{$start}>" placeholder="开始时间" id="start-time" onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})" autocomplete='off'>
                                <span class="input-group-addon">
                                    <i class="icon-calendar bigger-110"></i>
                                </span>
								<span class="input-group-addon" style="border: none !important;background-color:  inherit !important;">到</span>
								<input type="text" class="form-control" name="end" value="<{$end}>" placeholder="截止时间" onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})" autocomplete='off'>
								<span class="input-group-addon">
                                    <i class="icon-calendar bigger-110"></i>
                                </span>
							</div>
...
<div class="trade-list">
	<table class="ui-table-order" style="padding: 0px;">
		...
		<tr class="content-row">
			...
			<!-- 买家信息 -->
			<td class='cell'>
				<p>下单人：<a href="<{if $limit_trade == 1}>/wxapp/limit/sequenceOrder<{elseif $bargain_trade == 1}>/wxapp/bargain/sequenceOrder<{else}>/wxapp/sequence/tradeList<{/if}>?buyer=<{$val['t_buyer_nick']}>" class="new-window" target="_blank"><{$val['t_buyer_nick']}></a></p>
				...
				<{/if}>	
				<p>类型：<a href="<{if $limit_trade == 1}>/wxapp/limit/sequenceOrder<{elseif $bargain_trade == 1}>/wxapp/bargain/sequenceOrder<{else}>/wxapp/sequence/tradeList<{/if}>?postType=<{$val['t_express_method']}>" class="new-window" target="_blank"><{$expressMethod[$val['t_express_method']]}></a></p>
			</td>
			<!-- 团长信息 -->
			<td class="cell">
				<p>
					团长名称：
					<{if $val.t_se_leader}>
					<a href="<{if $limit_trade == 1}>/wxapp/limit/sequenceOrder<{elseif $bargain_trade == 1}>/wxapp/bargain/sequenceOrder<{else}>/wxapp/sequence/tradeList<{/if}>?leader=<{$val['asl_id']}>&leaderName=<{$val['asl_name']}>"class="new-window" title="<{$val['asl_name']}>">
						<{$val['asl_name']}>
					</a>
					<{else if $val.t_status != 7}>
					<!-- 团长信息未回写成功-执行手动操作的逻辑 -->
				...
				<p>社区名称：
					<a href="<{if $limit_trade == 1}>/wxapp/limit/sequenceOrder<{elseif $bargain_trade == 1}>/wxapp/bargain/sequenceOrder<{else}>/wxapp/sequence/tradeList<{/if}>?community=<{$val['asc_name']}>"class="new-window" title="<{$val['asc_name']}>">
			...
			<td class='cell'>
				<div class="td-cont text-center">
					...
					<p class="user-name">
						...
						<!--
						<span id="refund_<{$val['t_tid']}>" class="btn btn-danger btn-xs refund-btn"
						  data-tid="<{$val['t_tid']}>"
						  data-type="<{$val['t_type']}>"
						  data-feedback="<{$val['t_feedback']}>"
						  data-province="<{$val['ma_province']}>"
...
<div class="modal fade" id="excelOrderActivity" tabindex="-1" role="dialog" style="display: none;" aria-labelledby="myModalLabel" aria-hidden="true">
	<div class="modal-dialog" style="width: 700px;">
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-hidden="true">
				...
				<h4 class="modal-title" id="excelOrderActivityLabel">
					导出订单
				</h4>
			</div>
			<div class="modal-body" style="overflow: auto;text-align: center;margin-bottom: 45px">
				<div class="modal-plan p_num clearfix shouhuo">
					<form enctype="multipart/form-data" action="/wxapp/sequence/excelOrderActivity" method="post">
						<div class="form-group">
							<label class="col-sm-2 control-label">订单类型</label>
							<div class="col-sm-4">
								<select id="orderStatus" name="activity_orderStatus" class="form-control">
									<{foreach $link as $key=>$val}>
									<option value="<{$key}>"><{$val['label']}></option>
									<{/foreach}>
								</select>
							...
							<div class="col-sm-4" style="display: none;">
								<input type="checkbox" name="activity_addressOrder" checked style="display: inline-block;width: 25px;position: relative;top: 3px;font-size: 20px;">
								<label for="address-order" style="position: relative;top: 2px">根据地址排序</label>
							</div>
						...
						<div class="form-group">
							<label class="col-sm-2 control-label">配送方式</label>
							<div class="col-sm-10">
								<select id="activity_postType" name="activity_postType" class="form-control" style="width: 37%;">
									<option value="0">全部</option>
									<{foreach $expressMethod as $key=>$val}>
									<option value="<{$key}>"><{$val}></option>
									<{/foreach}>
						...
						<div class="form-group">
							<label class="col-sm-2 control-label">小区</label>
							<div class="col-sm-10">
								<select id="activity_communityId" name="activity_communityId" class="form-control" style="width: 37%;">
									<option value="0">全部</option>
									<{foreach $communitySelect as $val}>
									<option value="<{$val['id']}>"><{$val['name']}></option>
									<{/foreach}>
						...
						<div class="space"></div>
						<div class="form-group">
							<label class="col-sm-2 control-label">群组编号</label>
							<div class="col-sm-10">
								<input class="form-control" type="number" id="groupId" name="groupId" placeholder="请输入活动群组编号"/>
							</div>
						</div>
						...
						<div class="form-group">
							<label class="col-sm-2 control-label">活动名称</label>
							<div class="col-sm-10">
								<input class="form-control" type="text" id="activity_activitytitle" name="activity_activitytitle" placeholder="请输入活动名称,不填默认全部活动"/>
							</div>
						</div>
						<div class="space"></div>
						...
						<div class="form-group">
							<label class="col-sm-2 control-label">商品名称</label>
							<div class="col-sm-10">
								<input class="form-control" type="text" id="activity_goodsname" name="activity_goodsname" placeholder="请输入商品名称,不填默认全部商品"/>
							</div>
						</div>
						<div class="space"></div>
						<div class="form-group">
							<label class="col-sm-2 control-label">开始日期</label>
							<div class="col-sm-4">
								<input class="form-control date-picker" type="text" id="activity_startDate" data-date-format="yyyy-mm-dd" name="activity_startDate" placeholder="请输入开始日期"/>
							</div>
							<label class="col-sm-2 control-label">开始时间</label>
							<div class="col-sm-4 bootstrap-timepicker">
								<input class="form-control" type="text" id="timepicker1_activity" name="activity_startTime" placeholder="请输入开始时间"/>
							</div>
						</div>
						<div class="space"></div>
						<div class="form-group">
							<label class="col-sm-2 control-label">结束日期</label>
							<div class="col-sm-4">
								<input class="form-control date-picker" type="text" id="activity_endDate" data-date-format="yyyy-mm-dd" name="activity_endDate" placeholder="请输入结束日期"/>
							</div>
							<label class="col-sm-2 control-label">结束时间</label>
							<div class="col-sm-4 bootstrap-timepicker">
								<input class="form-control" type="text" id="timepicker2_activity" name="activity_endTime" placeholder="请输入结束时间"/>
							</div>
						</div>
						<div class="space" style="margin-bottom: 70px;"></div>
						<button type="button" class="btn btn-normal" data-dismiss="modal" style="margin-right: 30px">取消</button>
				...
				<div class="modal-plan p_num clearfix shouhuo">
					<form>
						...
						<div class="form-group">
							<label class="col-sm-2 control-label">开始日期</label>
							<div class="col-sm-4">
								<input class="form-control date-picker" type="text" id="print_startDate" data-date-format="yyyy-mm-dd" name="print_startDate" placeholder="请输入开始日期" autocomplete="off" />
							</div>
							<label class="col-sm-2 control-label">开始时间</label>
							<div class="col-sm-4 bootstrap-timepicker">
								<input class="form-control" type="text" id="timepicker_print_startTime" name="print_startTime" placeholder="请输入开始时间"/>
							</div>
						</div>
						<div class="space"></div>
						<div class="form-group">
							<label class="col-sm-2 control-label">结束日期</label>
							<div class="col-sm-4">
								<input class="form-control date-picker" type="text" id="print_endDate" data-date-format="yyyy-mm-dd" name="print_endDate" placeholder="请输入结束日期" autocomplete="off" />
							</div>
							<label class="col-sm-2 control-label">结束时间</label>
							<div class="col-sm-4 bootstrap-timepicker">
								<input class="form-control" type="text" id="timepicker_print_endTime" name="print_endTime" placeholder="请输入结束时间"/>
							</div>
						</div>
						<div class="space" style="margin-bottom: 70px;"></div>
						<button type="button" class="btn btn-normal" data-dismiss="modal" style="margin-right: 30px">取消</button>
