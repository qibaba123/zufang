...
<div  id="content-con">
    ...
    <div class="page-header search-box">
        <div class="col-sm-12">
            <form action="/wxapp/sequence/goodsCommunityEdit" method="get" class="form-inline">
                <div class="col-xs-11 form-group-box">
                    <div class="form-container">
                        <div class="form-group">
                            <div class="input-group">
                                <div class="input-group-addon">社区名称</div>
                                <input type="text" class="form-control" name="name" value="<{$name}>" placeholder="小区名称">
                            </div>
                        </div>
                        <div class="form-group">
                            <div class="input-group">
                                <div class="input-group-addon" style="height:34px;">所在区域</div>
                                <select id="area" name="area" style="height:34px;width:100%" class="form-control">
                                    <option value="0">全部</option>
                                    <{foreach $areaSelect as $key => $val}>
                                    <option <{if $area eq $key}>selected<{/if}> value="<{$key}>"><{$val['name']}></option>
                                    <{/foreach}>
                                </select>
                            </div>
                        </div>
                        <input type="hidden" id="hid_goods_id" name="id" value="<{$gid}>">
                    </div>
                </div>
    ...
    <div class="row">
        <div class='col-xs-12'>
            <p class='help-block'>
               <!-- * 默认所有社区可购买，选择指定社区后，仅勾选后的社区拥有购买权限。 -->
            </p>
        </div>
        <div class="col-xs-12">
            <div class="fixed-table-box" style="margin-bottom: 30px;">
                <div class="fixed-table-header">
                    <table class="table table-striped table-hover table-avatar table-title-darkbg">
                        <thead>
                            <tr>
                                <th class="center">
                                    <label>
                    ...
                    <table id="sample-table-1" class="table table-striped table-hover table-avatar" style="border: none">
                        ...
                        <{foreach $list as $val}>
                            <tr id="tr_<{$val['asc_id']}>">
                                <td class="center">
                                    <label>
                                        <input type="checkbox" class="ace" name="ids" value="<{$val['asc_id']}>"/>
                                        <span class="lbl"></span>
                                    </label>
                                </td>
                                <td >
                                ...
                                <td><{$areaSelect[$val['asc_area']]['name']}></td>
                                <td>
                                    <input type="hidden" value='<{$limit_community}>'>
                                    <{if $limit_community <= 0}>
                                        <span class="text-danger"><i class='icon-remove'></i>不可购买</span>
                                    <{else}>
                                        <{if $val['asgc_id'] > 0}>
                                        <span class="text-success"><i class='icon-ok'></i>可购买</span>
                                        <{else}>
                                        <span class='text-danger'><i class='icon-remove '>不可购买</i></span>
                                        <{/if}>
                                    <{/if}>
                                </td>
                                <td>
                                    <{if $val['asgc_id'] > 0}>
                                    <a href="javascript:;" class="btn btn-xs btn-warning select-community" data-type="remove" data-id="<{$val['asc_id']}>">移除社区可购权限</a>
                                    <{else}>
                                    <a href="javascript:;" class="btn btn-xs btn-success select-community" data-type="add" data-id="<{$val['asc_id']}>">设置此社区可购</a>
                                    <{/if}>
                                </td>
                            </tr>
                            <{/foreach}>
                            <tr><td colspan="2">
                                    <span class="btn btn-xs btn-name select-community-multi btn-danger" data-type="remove">批量移除</span>
                                    <span class="btn btn-xs btn-name select-community-multi btn-primary" data-type="add">批量添加</span>
...
<script type="text/javascript">
    ...
    $('.select-community').on('click',function () {
       ...
       var data = {
        ...
        $.ajax({
            'type'  : 'post',
            'url'   : '/wxapp/sequence/selectGoodsCommunitySingle',
            'data'  : data,
            'dataType' : 'json',
            'success'   : function(ret){
                layer.close(index);
    ...
    $('.select-community-multi').on('click',function(){
        var type = $(this).data('type');
        var ids  = get_select_all_ids_by_name('ids');
        if(!ids){
            layer.msg('请选择商品');
        ...
        if(ids && type){
            ...
            $.ajax({
                'type'  : 'post',
                'url'   : '/wxapp/sequence/selectGoodsCommunityMulti',
                'data'  : data,
                'dataType' : 'json',
                'success'   : function(ret){
                    layer.close(index);
