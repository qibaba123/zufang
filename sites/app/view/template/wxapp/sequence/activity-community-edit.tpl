...
<div  id="content-con">
    ...
    <div class="page-header search-box">
        <div class="col-sm-12">
            <form action="/wxapp/sequence/activityCommunityEdit" method="get" class="form-inline">
                    ...
                    <table id="sample-table-1" class="table table-striped table-hover table-avatar" style="border: none">
                        ...
                        <{foreach $list as $val}>
                            <tr id="tr_<{$val['asc_id']}>">
                                ...
                                <!--
                                <td id="show_num_<{$val['asc_id']}>">
                                    <span><{$val['asc_show_num']}></span>
                                    <a href="javascript:;" class="set-goodsinfo" data-id="<{$val['asc_id']}>" data-value="<{$val['asc_show_num']}>" data-field="show_num">修改</a>
                                </td>
                                <td id="stock_<{$val['asc_id']}>">
                                    <span><{$val['asc_stock']}></span>
                                    <a href="javascript:;" class="set-goodsinfo" data-id="<{$val['asc_id']}>" data-value="<{$val['asc_stock']}>" data-field="stock">修改</a>
                                </td>
                                <td id="sold_<{$val['asc_id']}>">
                                    <span><{$val['asc_sold']}></span>
                                    <a href="javascript:;" class="set-goodsinfo" data-id="<{$val['asc_id']}>" data-value="<{$val['asc_sold']}>" data-field="sold">修改</a>
                                </td>
                                -->
                                <td>
                                    <{if $val['asac_id'] > 0}>
                                    <a href="#" class="btn btn-xs btn-danger select-community" data-type="remove" data-id="<{$val['asc_id']}>">移除</a>
                                    <{else}>
                                    <a href="#" class="btn btn-xs btn-primary select-community" data-type="add" data-id="<{$val['asc_id']}>">添加</a>
                                    <{/if}>
                                </td>
                            </tr>
                            <{/foreach}>
...
<script type="text/javascript">
    ...
    $('.select-community').on('click',function () {
       ...
       var data = {
        ...
        $.ajax({
            'type'  : 'post',
            'url'   : '/wxapp/sequence/selectCommunitySingle',
            'data'  : data,
            'dataType' : 'json',
            'success'   : function(ret){
                layer.close(index);
    ...
    $('.select-community-multi').on('click',function(){
        ...
        if(ids && type){
            ...
            $.ajax({
                'type'  : 'post',
                'url'   : '/wxapp/sequence/selectCommunityMulti',
                'data'  : data,
                'dataType' : 'json',
                'success'   : function(ret){
                    layer.close(index);
