...
<div  id="mainContent">
    ...
    <div class="page-header" style="overflow:hidden">
        ...
        <{if $ac_type==32}>
            <div class="col-sm-1">
                <a target="_blank" class="btn btn-green btn-sm" href="/wxapp/configs/config#leaderwithdraw">
                    <i class="icon-cog bigger-40"></i> 提现配置
                </a>
            </div>
        <{else}>
            <div class="col-sm-1">
                <a class="btn btn-green btn-sm" href="/wxapp/city/withdrawCfg">
                    <i class="icon-cog bigger-40"></i> 提现配置
                </a>
            </div>
        <{/if}>
    ...
    <div ng-app="Withdraw"  ng-controller="WithdrawList">
        <div class="page-header search-box">
            <div class="col-sm-12">
                <form class="form-inline" action="/wxapp/city/withdraw">
        ...
        <div class="choose-state">
            <a href="/wxapp/city/withdraw" <{if !$audit}> class="active" <{/if}>>全部提现申请</a>
            <a href="/wxapp/city/withdraw?audit=audit" <{if $audit eq 'audit'}> class="active" <{/if}>>待审核</a>
            <a href="/wxapp/city/withdraw?audit=pass" <{if $audit eq 'pass'}> class="active" <{/if}>>已通过</a>
            <a href="/wxapp/city/withdraw?audit=refuse" <{if $audit eq 'refuse'}> class="active" <{/if}>>已拒绝</a>
        </div>
        <div class="row">
            <div class="col-xs-12">
                        ...
                        <table id="sample-table-1" class="table table-hover">
                            ...
                            <{foreach $list as $val}>
                                <tr>
                                    ...
                                    <{/if}>
                                    <td><{$val['wd_mobile']}></td>
                                    <td><{if isset($withdrawType[$val['wd_type']])}><{$withdrawType[$val['wd_type']]}><{/if}><{if $val['wd_sorce'] == 2}><br>商品垫付提现<{/if}></td>
                                    <td>
                                        <{if $val['wd_type'] == 3}>
                                    ...
                                    <td>
                                        ...
                                        <{elseif $val['wd_audit'] == 3}>
                                        <span class="">会计审核中</span>
                                        <{/if}>
                                    </td>
    ...
    $(function(){
        ...
        $('.modal-save').on('click',function(){
            ...
            if(id && status){
                ...
                $.ajax({
                    'type'  : 'post',
                    'url'   : '/wxapp/city/dealWithdraw',
                    'data'  : data,
                    'dataType'  : 'json',
                    success : function(json_ret){
                        layer.msg(json_ret.em,{time: 1600},function () {
                            layer.close(index);
                            if(json_ret.ec == 200){
