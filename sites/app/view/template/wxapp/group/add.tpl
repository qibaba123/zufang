					...
					<div class="form-group">
						...
						<div class="col-xs-6">
							<select class="form-control selectpicker chosen-select" id="g_id" name="g_id" onchange="changePrice()"  <{if $row && $row['gb_g_id']}>disabled="disabled"<{/if}>  data-live-search="true"  data-need="required" data-placeholder="请选择参团商品">
								...
								<option data-price="<{$val['g_price']}>" value="<{$val['g_id']}>" <{if $row && $row['gb_g_id'] eq $val['g_id']}>selected="selected"<{/if}>><{mb_substr($val['g_name'],0,20)}></option>
								<{/foreach}>
							</select>
						</div>
			...
			<div class="ptinfo-fenlei clearfix">
				...
				<div class="col-xs-10" style="border-left:1px dashed #ddd;">
					<div class="form-group">
						...
						<div class="col-xs-9">
							<input id="startTime" name="startTime" type="text" placeholder="请选择开始时间" class="Wdate form-control" <{if $row && $row['gb_start_time']}>disabled="disabled"<{/if}> value="<{if $row}><{date('Y-m-d H:i:s',$row['gb_start_time'])}><{/if}>" onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',maxDate:'#F{$dp.$D(\'endTime\');}'})">
							<!-- <div class="col-xs-6">
					...
					<div class="form-group">
						...
						<div class="col-xs-9">
							<input id="endTime" name="endTime"  type="text" placeholder="请选择结束时间" class="Wdate form-control" <{if $row && $row['gb_end_time']}>disabled="disabled"<{/if}> value="<{if $row}><{date('Y-m-d H:i:s',$row['gb_end_time'])}><{/if}>" onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',minDate:'#F{$dp.$D(\'startTime\');}'})">
							<!-- <div class="col-xs-6">
