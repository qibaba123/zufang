            ...
            <div class="table-responsive" id="content-con">
                <table id="sample-table-1" class="table table-hover table-button">
                    ...
                    <{foreach $list as $key => $val}>
                        <tr id="tr_<{$val['agg_id']}>" <{if ($key % 2 == 1)}>class="info"<{else}><{/if}>>
                            <td id="name_<{$val['agg_id']}>"><{$val['agg_name']}></td>
                            <td>
                                <{$val['agg_total']}>
                                <div class="pull-right" style="font-size:0">
                                    <{if $val['agg_total'] > 0}>
                                    <a class="btn btn-xs btn-success btn-goods" href="javascript:;" data-mk="look" data-id="<{$val['agg_id']}>">查看</a>
                                    <{/if}>
                                    <a class="btn btn-xs btn-info btn-goods" href="javascript:;" data-mk="add"  data-id="<{$val['agg_id']}>">增加关联</a>
                                </div>
                            </td>
                            <td><{date('Y-m-d H:i',$val['agg_create_time'])}></td>
                            <td style="color:#ccc;">
                                <a class="btn-group" href="javascript:;" data-mk="edit" data-id="<{$val['agg_id']}>" data-name="<{$val['agg_name']}>" data-style="<{$val['agg_list_type']}>" data-img="<{$val['agg_bg']}>" data-brief="<{$val['agg_brief']}>">编辑</a>
                                -
                                <a class="btn-group" href="javascript:;" data-mk="del" data-id="<{$val['agg_id']}>" style="color:#f00;">删除</a>

                                <!--<a href="javascript:;" id="link_<{$val['agg_id']}>" class="btn-link" data-link="<{$val['link']}>">链接</a>-->
