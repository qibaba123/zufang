...
<!--新版布局开始-->
			...
			<div class="sell-status common-content">
				<div class="title-wrap flex-wrap">
					...
					<div class="sell-money">
						<span class="num-wrap">今日订单金额<span class="num" id="sellMoney"><{$todaySaleMoneyTotal}></span></span>
						<span class="num-wrap">今日售出商品数<span class="num" id="goodsNum"><{$todaySaleGoodsTotal}></span></span>
					</div>
				</div>
				<div class="tab-wrap flex-wrap">
					<div class="common-tab flex-con">
						<span class="tab-item goods-money-tab active" data-type="goods">售出商品数</span>
						<span class="tab-item goods-money-tab" data-type="money">销售金额</span>
					</div>
	...
	<div class="row margin-bottom">
		...
		<div class="col-lg-8 col-md-12 col-sm-12 col-xs-12" style="padding:0;">
			<div class="growth-rate common-content">
				<div class="tab-wrap flex-wrap">
					<div class="common-tab flex-con">
					...
					<div class="status-tips">
						<span class="tip"><span class="color-lump color-blue"></span><span class="this-tab-tips">今日</span></span>
						<span class="tip"><span class="color-lump color-green"></span><span class="last-tab-tips">昨日</span></span>
					</div>
				</div>
				<div class="rate-chart" id="rate-chart"></div>
				<div class="rate-change flex-wrap day-month-tips">
					<div class="change-item flex-con"><img src="/public/wxapp/analysis/image/icon_<{if $dayAddMemberPercent>=0}>up<{else}>down<{/if}>.png" alt="" /><span><{$dayAddMemberPercent}>%</span></div>
					<div class="change-item flex-con"><img src="/public/wxapp/analysis/image/icon_<{if $dayOrderPercent>=0}>up<{else}>down<{/if}>.png" alt="" /><span><{$dayOrderPercent}>%</span></div>
					<div class="change-item flex-con"><img src="/public/wxapp/analysis/image/icon_<{if $dayCommunityPercent>=0}>up<{else}>down<{/if}>.png" alt="" /><span><{$dayCommunityPercent}>%</span></div>
					<div class="change-item flex-con"><img src="/public/wxapp/analysis/image/icon_<{if $dayRefundPercent>=0}>up<{else}>down<{/if}>.png" alt="" /><span><{$dayRefundPercent}>%</span></div>
				</div>
				<div class="total-num">
					<div class="num-wrap">总用户数<span class="num"><{$totalMember}></span></div>
					<div class="num-wrap">总订单数<span class="num"><{$orderTotal}></span></div>
					<div class="num-wrap">总小区数<span class="num"><{$allCommunityTotal}></span></div>
					<div class="num-wrap">总维权订单<span class="num"><{$refundTotal}></span></div>
				</div>
			</div>
		</div>
		<!--增长率情况结束-->
	...
	<div class="row">
		<!--小区排行榜开始-->
		<div class="col-lg-8 col-md-12 col-sm-12 col-xs-12" style="padding:0;">
			<div class="community-rank common-content">
				<div class="tab-wrap flex-wrap">
					<div class="common-tab flex-con">
						<span class="tab-item community-time-change active" data-type="month">本月小区排行榜</span>
						<span class="tab-item community-time-change" data-type="all">小区总排行榜</span>
					</div>
					<div class="status-tips">
				...
				<div class="community-wrap">
					<div class="community-tab flex-wrap">
						...
						<div class="tab-item flex-con" style="text-align: center;">小区详情</div>
					</div>
					<div class="community-list">
						...
						<div class="community-item flex-wrap">
							...
							<div class="community-infor community-width flex-wrap">
								<img src="<{$val['m_avatar']}>" class="avatar" alt="头像" />
								<div class="infor flex-con">
									<div class="name"><{$val['asc_name']}></div>
									<div class="tz">团长：<{$val['asl_name']}></div>
								</div>
							</div>
							<div class="con flex-con"><{$val['nums']}></div>
							<div class="con flex-con"><{$val['total']}></div>
							<div class="con flex-con"><a href="/wxapp/sequence/communityEdit?id=<{$val['asc_id']}>">查看</a></div>
						</div>
                        <{/foreach}>
					</div>
		...
		<div class="order-user-wrap col-lg-4 col-md-12 col-sm-12 col-xs-12" style="padding-right:0;">
			<div class="order-user common-content">
				...
				<div class="total-num">
					<div class="num-wrap">新增用户数<span class="num">446</span></div>
					<div class="num-wrap">新增订单数<span class="num">121</span></div>
				</div>
			</div>
		</div>
		<!--近15日订单量-新增用户数结束-->
...
<script>
	...
	$(function(){
		...
		$('.today-tab').click(function () {
			$('.this-tab-tips').text('今日');
            $('.last-tab-tips').text('昨日');
            ...
            chart3([<{$todayNewMember}>, <{$todayOrderTotal}>, <{$todayCommunityTotal}>, <{$todayRefundTotal}>], [<{$yesterdayNewMember}>, <{$yesterdayOrderTotal}>, <{$yesterdayCommunityTotal}>, <{$yesterdayRefundTotal}>]);
        });

        $('.month-tab').click(function () {
            $('.this-tab-tips').text('本月');
            $('.last-tab-tips').text('上月');
            ...
            chart3([<{$thisMonthMember}>, <{$thisMonthOrderTotal}>, <{$thisMonthCommunityTotal}>, <{$thisMonthRefundTotal}>], [<{$lastMonthMember}>, <{$lastMonthOrderTotal}>, <{$lastMonthCommunityTotal}>, <{$lastMonthRefundTotal}>]);
        });

		$(".fifteen-order-change").click(function () {
            chart4(<{$daysArr}>,<{$fifteenOrderStatistic}>);
		});
		...
		$(".goods-money-tab").click(function () {
            let type = $(this).data('type');
            let atype = $('.data-type-tab.active').data('type');
            getTimeGroupOrderStatistic(type, atype);
        });

        $(".day-type-tab").click(function () {
            let type = $(this).data('type');
            let atype = $('.goods-money-tab.active').data('type');
            getTimeGroupOrderStatistic(atype, type);
        });
        ...
        $(".community-sale-change").click(function () {
            let type = $(this).data('type');
            let atype = $('.community-time-change.active').data('type');
            if(type == 'nums' && atype == 'month'){
                $(".community-list").eq(0).show().siblings('.community-list').hide()
            }
            if(type == 'nums' && atype == 'all'){
                $(".community-list").eq(1).show().siblings('.community-list').hide()
            }
            if(type == 'money' && atype == 'month'){
                $(".community-list").eq(2).show().siblings('.community-list').hide()
            }
            if(type == 'money' && atype == 'all'){
                $(".community-list").eq(3).show().siblings('.community-list').hide()
            }
        });
		...
		//加载图表
        getTimeGroupOrderStatistic('goods', 'today');
	    chart2();
	    chart3([<{$todayNewMember}>, <{$todayOrderTotal}>, <{$todayCommunityTotal}>, <{$todayRefundTotal}>], [<{$yesterdayNewMember}>, <{$yesterdayOrderTotal}>, <{$yesterdayCommunityTotal}>, <{$yesterdayRefundTotal}>]);
	    chart4(<{$daysArr}>,<{$fifteenOrderStatistic}>);
		...
		setTimeout(function(){
			...
			$('#sellMoney').animateNumber(
			  {
			    number: <{$todaySaleMoneyTotal}> > 0 ? <{$todaySaleMoneyTotal}> : 0,
	...
	function getTimeGroupOrderStatistic(type, time) {
        ...
        $.ajax({
            'type'   : 'post',
			'data' : {type: time},
            'url'   : '/wxapp/statisticanalysis/sequenceTimeGourpOrderStatistic',
            'dataType'  : 'json',
...
//		        name:'收入',
    ...
    function chart3(thisData, lastData){
		// 基于准备好的dom，初始化echarts实例
