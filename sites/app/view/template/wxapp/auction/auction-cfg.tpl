...
<script>
    ...
    app.controller('chCtrl', ['$scope','$http','$timeout',function($scope,$http,$timeout){
        $scope.headerTitle= "<{$tpl['aai_title']}>" ? "<{$tpl['aai_title']}>" : "拍卖" ;
        $scope.listTitle= "<{$tpl['aai_list_title']}>" ? "<{$tpl['aai_list_title']}>" : "拍卖列表" ;
        $scope.orderTitle= "<{$tpl['aai_order_title']}>" ? "<{$tpl['aai_order_title']}>" : "我的竞拍" ;
        $scope.banners = <{$slide}>;
        $scope.phone   = "<{$tpl['aai_phone']}>";
        $scope.confirmTime = '<{$tpl['aai_confirm_time']}>' ? '<{$tpl['aai_confirm_time']}>' : '7';
        $scope.serviceStartTime = "<{$tpl['aai_service_start_time']}>";
        $scope.serviceEndTime = "<{$tpl['aai_service_end_time']}>";
