...
<div class="version-manage">
    <div class="version-item-box">
        <div class="code-version-title">
            <h3>语录信息</h3>
            <!--
            <a href="#" data-toggle="modal" data-target="#myModal" onclick="replyComment(0, 0, '<{$row['kcq_id']}>')" >回复</a>
            <div style="float:right;width: 300px">
                <span style="font-size: 16px">语录审核<span style="font-size: 10px">（如果封禁语录将不会在手机端显示）</span></span>
                <span class='tg-list-item'>
                     <input class='tgl tgl-light' id='post_status' type='checkbox' onchange="postStatusChange()" <{if $row && $row['kcq_status']}>checked<{/if}> >
    ...
    <div class="version-item-box" style="width: 90%;margin-left: 5%;">
        ...
        <div style="width: 100%;text-align: right;margin-bottom: 5px;display: none">如果封禁评论将不会在手机端显示</div>
        <div id="content-con">
            <div  id="mainContent" >
                <div class="row">
                    <div class="col-xs-12">
                            ...
                            <table id="sample-table-1" class="table table-striped table-hover table-avatar">
                                ...
                                <{foreach $commentList as $val}>
                                    <tr id="tr_<{$val['kcq_id']}>">
                                        <td style="max-width: 120px"><{if $val['akqc_m_id'] == -1}><{$curr_shop['s_name']}><{else}><{$val['m_nickname']}><{/if}></td>
                                        <td>
                                            <{if $val['akqc_reply_mid']>0 && $val['rm_nickname']}><a href="#"><{$val['rm_nickname']}></a>
                                            <{else}>
                                                <{if $val['akqc_reply_mid'] == -1}>
                                                <{$curr_shop['s_name']}>
                                                <{else}>
                                                无
                                                <{/if}>
                                        ...
                                        <td style="max-width: 200px;white-space: normal;overflow: hidden"><{$val['akqc_comment']}></td>
                                        <td><{if $val['akqc_create_time'] > 0}><{date('Y-m-d H:i',$val['akqc_create_time'])}><{/if}></td>
                                        <td>
                                            <!--
                                            <a href="#" data-toggle="modal" data-target="#myModal" onclick="replyComment('<{$val['akqc_id']}>', '<{$val['akqc_m_id']}>', '<{$val['akqc_q_id']}>')" >回复</a>-
                                            -->
                                            <a href="#" id="delete-confirm" onclick="deletePostComment('<{$val['akqc_id']}>')" style="color: red">删除</a>
                                        </td>
                                        <!--
                                        <td>
                                            <span class='tg-list-item'>
...
//        });
    ...
    function deletePostComment(id) {
        layer.confirm('确定删除吗？', {
            ...
            $.ajax({
                'type'   : 'post',
                'url'   : '/wxapp/knowledgepay/deleteQuotationComment',
                'data'  : { id:id},
                'dataType'  : 'json',
                'success'  : function(ret){
                    layer.close(load_index);
