...
<{/if}>
<div class="page-header search-box" <{if $pointOrder}>
	style="margin-left:150px;"
	<{/if}>>
	<div class="col-sm-12">
				...
				<div class="form-container">
					...
					<div class="form-group">
						<div class="input-group ">
							...
							<input type="text" class="form-control" name="title" value="<{$title}>"  placeholder="课程名称">
						</div>
					</div>
					<{if $indexTpl == 55}>
					<div class="form-group">
...
<!-- 订单汇总信息 -->
<div class="balance clearfix" style="border:1px solid #e5e5e5;margin-bottom: 20px; <{if $pointOrder}>margin-left:150px;<{/if}>">
...
<div class="choose-state" <{if $pointOrder}>
	style="margin-left:150px;"
	<{/if}>>
	<{foreach $link as $key=>$val}>
	<a href="/wxapp/knowledgepay/pointOrder?status=<{$key}>" <{if $status && $status eq $key}>class="active"<{/if}>><{$val['label']}></a>
	<{/foreach}>
</div>
<div class="trade-list" <{if $pointOrder}>
	style="margin-left:150px;"
	<{/if}>>
	<table class="ui-table-order" style="padding: 0px;">
		<thead class="js-list-header-region tableFloatingHeaderOriginal" style="position: static; top: 0px; margin-top: 0px; left: 225px; z-index: 1; width: 794px;">
		    <tr class="widget-list-header">
		        <th class="" colspan="2" style="min-width: 212px; max-width: 212px;">课程</th>
			...
			<td colspan="2" class="text-right">
				<div class="order-opts-container">
					<div class="js-opts" style="display: block;">
						<a href="/wxapp/knowledgepay/tradeDetail?order_no=<{$val['t_tid']}>" class="new-window" >查看详情</a>
		...
		<tr class="content-row">
			...
			<td class="title-cell">
				<p class="goods-title">
					<a href="/wxapp/knowledgepay/tradeList?title=<{$val['t_title']}>"class="new-window" title="<{$val['t_title']}>">
						<{$val['t_title']}>
					</a>
				</p>
				<p>
			...
			<td class="customer-cell" rowspan="1">
				<p>
					<a href="/wxapp/knowledgepay/tradeList?buyer=<{$val['t_buyer_nick']}>" class="new-window" target="_blank">
						<{$val['t_buyer_nick']}>
					</a>
				</p>
	...
	<div class="modal fade" id="agreeTK" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-body">
					<div class="alert">
						订单中的部分课程，买家已提交了退款申请。你需要先跟买家协商，买家撤销退款申请后，才能进行发货操作。
					</div>
				</div>
				<div class="modal-footer">
				...
				<div class="modal-plan p_num clearfix shouhuo">
					<form enctype="multipart/form-data" action="/wxapp/order/excelOrder" method="post">
						...
						<div class="form-group">
							...
							<div class="col-sm-10">
								<input class="form-control" type="text" id="goodstitle" name="goodstitle" placeholder="请输入课程名称,不填默认全部课程"/>
							</div>
						</div>
						<div class="space"></div>
						<div class="form-group">
