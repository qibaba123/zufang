...
<head>
    ...
    <style>
        ...
        *{box-sizing: border-box;}
        ...
        .login-part .right-login .left-login-con { font-size: 14px; width: 400px; display: inline-block; vertical-align: top; padding: 15px 20px; box-sizing: border-box; position: relative; }
...
<div class="login-part-wrap">
                ...
                <div class="left-login-con">
                    ...
                    <div class="login-part-box js_login js_pass_login show" ng-controller="MemberLoginController">
                        <form name="member_login_form" >
                            <div class="label-name">天店通后台管理系统更新程序</div>
                            <div class="login-input-wrap">
                                <div class="input-box">
                                    <input required="required" type="text" id="install-code" class="login-input" placeholder="更新码" />
                                </div>
                            </div>
                            <button type="submit" class="login-btn setup-btn">开始更新</button>
                        </form>
                    </div>
                </div>
    ...
    <div class="copy-rights">2019 © 郑州天点科技有限公司 · All Rights Reserved <{#domain_icp#}></div>
</div>
...
<script>
    $('.setup-btn').on('click',function(){
        ...
        if(code && host && name && user && passwd){
            layer.confirm('将执行安装程序，请不要关闭或刷新页面', {
                ...
                analysisApplet(type, code, host, name, user, passwd);
            });
        }else{
        ...
        $.ajax({
            ...
            success : function(data){
                if(data.ec == 200){
                    ...
                    analysisWxapp(type, code, host, name, user, passwd);
                }else{
    ...
    function analysisWxapp(type, code, host, name, user, passwd) {
        $('.analysis-wxapp').show();
        ...
        $.ajax({
            ...
            success : function(data){
                if(data.ec == 200){
                    ...
                    analysisView(type, code, host, name, user, passwd);
                }else{
    ...
    function analysisView(type, code, host, name, user, passwd) {
        $('.analysis-view').show();
        ...
        $.ajax({
            ...
            success : function(data){
                if(data.ec == 200){
                    ...
                    analysisModel(type, code, host, name, user, passwd);
                }else{
    ...
    function analysisModel(type, code, host, name, user, passwd) {
        $('.analysis-model').show();
        ...
        $.ajax({
            ...
            success : function(data){
                if(data.ec == 200){
                    ...
                    analysisMysql(type, code, host, name, user, passwd);
                }else{
    ...
    function analysisMysql(type, code, host, name, user, passwd) {
        $('.analysis-sql').show();
        ...
        $.ajax({
            ...
            success : function(data){
                if(data.ec == 200){
                    ...
                    analysisZip(type, code, host, name, user, passwd);
                }else{
    ...
    function analysisZip(type, code, host, name, user, passwd) {
        $('.zip-package').show();
    ...
    function analysisSetup(zipname, shoudong=0) {
        $('.setup-package').show();
        $('.setup-package .label').css('color', '#5183fc');
        $.ajax({
            'type'  : 'post',
            'url'   : '/update/setup',
            'data'  : {zipname, shoudong},
            'dataType' : 'json',
            success : function(data){
                if(data.ec == 200){
              ...
              layer.confirm('手动下载 https://www.tiandiantong.com/public/build/'+zipname+", 放置到根目录下的installcode文件夹下，并重命名为tdtcode.zip，点击开始安装", {
                ...
                analysisSetup(zipname, 1)
              })
            }
        });
    }
  
    function delZipFile(zipname){
       $.ajax({
            'type'  : 'post',
