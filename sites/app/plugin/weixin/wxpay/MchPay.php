 ...
 * Time: 下午2:25
 */
class App_Plugin_Weixin_Wxpay_MchPay {
    ...
    private $unified_url    = "https://api.mch.weixin.qq.com/sandboxnew/pay/unifiedorder";
    private $query_url      = "https://api.mch.weixin.qq.com/sandboxnew/pay/orderquery";
    private $refund_url     = "https://api.mch.weixin.qq.com/sandboxnew/pay/refund";
    private $q_refund_url   = "https://api.mch.weixin.qq.com/sandboxnew/pay/refundquery";
    private $bill_url       = "https://api.mch.weixin.qq.com/sandboxnew/pay/downloadbill";
    ...
    public function unifiedJsapiOrder($openid, $body, $tid, $amount, $notify_url, $other = array()) {
       ...
       /* $mch_id     = $this->wx_pay['wp_mchid'];//商户ID
        ...
        $appkey = $this->_get_sign_key($mch_id, $request_params['nonce_str'], $mch_key);
        $mch_key    = $appkey['app_key'];
     ...
     * 查询订单
     */
    public function queryPayOrder($tid) {
        /*if (!$this->has_ready) {
            return -1;
        }
        $mch_id     = $this->wx_pay['wp_mchid'];//商户ID
        ...
        Libs_Log_Logger::outputLog($request_params);
        if ($xml = $this->toXml($request_params)) {
            $ret = self::postXmlCurl($xml, $this->query_url);
            Libs_Log_Logger::outputLog($ret);
            $ret = $this->fromXml($ret);
            if ($ret) {
                if ($ret['return_code'] == 'SUCCESS' && $ret['result_code'] == 'SUCCESS') {
     ...
     * @param string $num_type  'sh'/'wx' $tid为商户单号,还是微信单号
     */
    public function queryRefundOrder($tid, $num_type = 'sh') {
        /*if (!$this->has_ready) {
            return -1;
        }
        ...
        $mch_key    = $appcfg['ap_mchkey'];//商户key
        $wx_appid   = $appcfg['ap_appid'];//公众号ID*/
        $dhtype     = $num_type == 'sh' ? "out_trade_no" : "refund_id";

        $request_params = array(
            'appid'             => $wx_appid,//公众号ID
            'mch_id'            => $mch_id,//商户号
            'nonce_str'         => self::getNonceStr(24),
            $dhtype             => $tid,//商户内部订单号或微信退款单号
        );
        $appkey = $this->_get_sign_key($mch_id, $request_params['nonce_str'], $mch_key);
        $mch_key    = $appkey['app_key'];
        ...
        if ($xml = $this->toXml($request_params)) {
            $ret = self::postXmlCurl($xml, $this->q_refund_url);
            $ret = $this->fromXml($ret);
            Libs_Log_Logger::outputLog($ret);
            if ($ret) {
                if ($ret['return_code'] == 'SUCCESS' && $ret['result_code'] == 'SUCCESS') {
    ...
    /*
     * 下载对账单
     * @param string $bill_date 具体到日期,如20170117
     * @param string $bill_type [ALL,SUCCESS,REFUND,RECHARGE_REFUND]
     */
    public function downloadPayBill($bill_date, $bill_type = 'ALL') {
        /*if (!$this->has_ready) {
            return -1;
        }
        $mch_id     = $this->wx_pay['wp_mchid'];//商户ID
        ...
        if ($xml = $this->toXml($request_params)) {
            $ret = self::postXmlCurl($xml, $this->bill_url);
            Libs_Log_Logger::outputLog($ret);
            if ($ret) {
                return $ret;
            }
    ...
    private function _get_sign_key($mch_id, $nonce_str, $mch_key) {
        $sign  = self::makeWxpaySign(array('mch_id' => $mch_id, 'nonce_str' => $nonce_str), $mch_key);
        $request_params = array(
            'mch_id'            => $mch_id,//商户号
            'nonce_str'         => $nonce_str,
            'sign'              => $sign,
        ...
        Libs_Log_Logger::outputLog($request_params);
        if ($xml = $this->toXml($request_params)) {
            $ret = self::postXmlCurl($xml, 'https://api.mch.weixin.qq.com/sandboxnew/pay/getsignkey');
            Libs_Log_Logger::outputLog($ret);
            $ret = $this->fromXml($ret);
            if ($ret) {
...
//        $appcfg = $applet_storage->findShopCfg();
     ...
     * 小程序校验回调支付通知（买家版订单支付）
     */
    public function notifyVerifyApplet($code) {
        // 获取小程序配置及支付相关配置
//        $applet_storage = new App_Model_Applet_MysqlCfgStorage($this->sid);
//        $appcfg = $applet_storage->findShopCfg();
        $appletPay_Model = new App_Model_Applet_MysqlAppletPayStorage($this->sid);
     ...
     * @param string $num_type  'sh'/'wx' $tid为商户单号,还是微信单号
     */
    public function appletRefundPayOrder($tid, $rfid, $total_fee, $refund_fee, $num_type = 'sh') {
        $total_fee  = round($total_fee*100);//转化为分
        $refund_fee = round($refund_fee*100);//转化为分
...
//        $wx_appid   = $this->wx_pay['wp_appid'];//公众号ID
        ...
        if ($xml = $this->toXml($request_params)) {
            ...
            }elseif ($ret['return_code'] == 'SUCCESS' && $ret['result_code'] == 'FAIL'){   // 提交成功但业务结果处理失败
                return array(
                    'code'   => 'FAIL',
