 ...
 * Time: 上午9:48
 */
class App_Plugin_Weixin_SubPay {
    ...
    public function __construct($sid) {
        ...
        $this->fxb_pay_cfg  = plum_parse_config('sub_pay', 'weixin');
        //提现相关配置
     ...
     * @return array|int
     */
    public function agentPayRecharge($amount, $tid, $notify_url, $body, $other=array()){

        $amount     = abs($amount);//单位为分
        $body       = mb_strlen($body, 'UTF-8') > 40 ? mb_substr($body, 0, 40, 'UTF-8') : $body;

        $payCfg = plum_parse_config('fxb_pay','weixin');
        if(!$payCfg){
            return 40005 ;    // 未配置微信支付
        }
        $request_params = array(
            'appid'             => $payCfg['app_id'], //小程序ID
