    ...
    public function indexAction(){
        ...
        if($this->shop){
            ...
            $info['data'] = array(
                ...
                'phoneOpen'      => intval($this->applet_cfg['ac_phone_open']),   // 电话按钮是否显示
                ...
                'recommendGoods' => $this->_recommend_goods_list($this->applet_cfg['ac_index_tpl']),   // 首页推荐商品,
                'kinds'          => $this->_shop_kind_list($this->applet_cfg['ac_index_tpl']),         // 首页商品分类
                'limit'          => $this->_limit_activity_list(),
                'group'          => $this->_fetch_group_list(),//首页拼团区块
                'kind2Goods'     => $this->_get_kind2_goods_data($this->applet_cfg['ac_index_tpl']),
                'categoryGoods'  => $this->_get_shop_category_good(),
                'notice'         => $tplData['notice']?$tplData['notice']:array(),
                'noticeList'     => $this->_get_shop_notice_list($this->applet_cfg['ac_index_tpl']),
                'lottery'        => $this->_check_lottery_now(),
                'supportOpen'    => $this->support && isset($this->support['as_audit']) ? intval($this->support['as_audit']) : 0,
                'distribOpen'    => $this->_find_shop_three_distrib(),
                'clothesStatus'  => $this->shop['s_clothes_status'],
                'shopInfo'        => $this->shop_company_info(),
                'popup'          => $this->_get_popup(),
                'meiqiaId'      => isset($this->applet_cfg['ac_meiqia_id']) && $this->applet_cfg['ac_meiqia_id'] ? $this->applet_cfg['ac_meiqia_id'] : ''
            );
            $member_model  = new App_Model_Member_MysqlMemberCoreStorage();
            $member_row    = $member_model->getRowById($uid);
            $info['data']['is_area'] = $member_row['m_is_area'];

            $hadCouponIndex = plum_get_isset_param('couponIndex');//防止二次调用首页接口导致优惠券提前领取
            $couponIndex    = $this->request->getIntParam('couponIndex');
            if($hadCouponIndex){
                if($uid && $couponIndex == 1){
            ...
            if($this->appletType==5){
                $info['data']['bottom_menu'] = $this->_get_bottom_menu();
            }
