<?php /* Smarty version Smarty-3.1.17, created on 2020-01-13 11:22:54
         compiled from "/mnt/www/default/ddfyce/yingxiaosc/sites/app/view/template/site/install.tpl" */ ?>
<?php /*%%SmartyHeaderCode:15013290505e1be28eeab507-97293357%%*/if(!defined('SMARTY_DIR')) exit('no direct access allowed');
$_valid = $_smarty_tpl->decodeProperties(array (
  'file_dependency' => 
  array (
    '1ecf446d252d1990ac64d776887739d23549f2ce' => 
    array (
      0 => '/mnt/www/default/ddfyce/yingxiaosc/sites/app/view/template/site/install.tpl',
      1 => 1574217863,
      2 => 'file',
    ),
  ),
  'nocache_hash' => '15013290505e1be28eeab507-97293357',
  'function' => 
  array (
  ),
  'variables' => 
  ...
  'unifunc' => 'content_5e1be28eee1c85_33884237',
),false); /*/%%SmartyHeaderCode%%*/?>
<?php if ($_valid && !is_callable('content_5e1be28eee1c85_33884237')) {function content_5e1be28eee1c85_33884237($_smarty_tpl) {?><!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <title>天店通小程序自动部署系统</title>
    ...
    <style>
        ...
        *{box-sizing: border-box;}
        ...
        .top-opera-box { font-size: 0; text-align: right; padding: 8px 0; }
        ...
        .login-part .right-login { background-color: #fff;font-size: 0;width: 701px;margin: auto; }
        ...
        .precent-wrap{height:8px;border-radius:8px!important;background-color:#DFDFDF;position:relative;margin-top:5px}
	    .precent-wrap.points-precent{background-color:#E8F4F0;}
	    .precent-wrap .precent-bg{height:8px;border-radius:8px!important;background-color:#1285EF;position:absolute;left:0;bottom:0;text-align: right;color:#fff;line-height:15px;letter-spacing: 0;font-size:12px;}
	    .precent-wrap.points-precent .precent-bg{background-color:#45E3AB;}
        .progress-list{margin-top: 13px;}
        .progress-list .list-item{position: relative;}
        .progress-list .progress-title .icon{display: inline-block;height: 10px;width: 10px;border: 1px solid #1285EF;border-radius: 100%;margin-right: 5px;position: relative;top: -1px;}
        .progress-list .progress-title .label{font-size: 13px;color: #aaa;font-weight: normal;width: 230px;display: inline-block;white-space: normal;text-align: left;line-height: 1.5;}
        .progress-list .progress-label{ width: 1px;height: 20px;background: #5183fc;position: absolute;top: -11px;left: 4px;}
        .progress-list .progress-title .icon.ing{border:0; height: 13px; width: 12px}
        .progress-list .progress-title .icon.ing{-webkit-transition-property: -webkit-transform;-webkit-transition-duration: 1s;-moz-transition-property: -moz-transform;-moz-transition-duration: 1s;-webkit-animation: rotate 1s linear infinite;-moz-animation: rotate 1s linear infinite;-o-animation: rotate 1s linear infinite;animation: rotate 1s linear infinite;}
        ...
        .layui-layer-content {user-select: text;}
    </style>
</head>
<body>
                ...
                <div class="left-login-con">
                    ...
                    <div class="login-part-box js_login js_pass_login show" ng-controller="MemberLoginController">
                        <form name="member_login_form" >
                            <div class="label-name">天店通后台管理系统安装程序</div>
                            <div class="login-input-wrap">
                                <div class="input-box">
                                    <input required="required" type="text" class="login-input" id="install-code" placeholder="安装码" />
                                </div>
                                <div class="input-box">
                                    <input required="required" type="text" class="login-input" id="db-host" placeholder="数据库地址" value="<?php echo $_smarty_tpl->tpl_vars['externalIp']->value;?>
"/>
                                </div>
                                <div class="input-box">
                                    <input required="required" type="text" class="login-input" id="db-name" placeholder="数据库名称" value="<?php echo $_smarty_tpl->tpl_vars['dbname']->value;?>
"/>
                                </div>
                                <div class="input-box">
                                    <input required="required" type="text" class="login-input" id="db-user" placeholder="数据库账号" value="<?php echo $_smarty_tpl->tpl_vars['dbuser']->value;?>
"/>
                                </div>
                                <div class="input-box">
                                    <input required="required" type="text" class="login-input" id="db-passwd" placeholder="数据库密码" value="<?php echo $_smarty_tpl->tpl_vars['dbpasswd']->value;?>
"/>
                                </div>
                            </div>
                            <butto type="submit" class="login-btn setup-btn">开始安装</button>
                        </form>
                    </div>
                </div>
                <div class="right-code-show">
                    <div class="help-contact">
                         <div class="progress-title"><span class="progress-header-title">等待安装</span>（<span class="progress-num">0</span>%）</div>
                         <div class="precent-wrap">
                            <div class="precent-bg" style="width:0%;"></div>
                         </div>
                         <div class="progress-list">
                            <div class="list-item analysis-applet" style="display:none">
                              <div class="progress-title">
                                <span class='icon over' style="display:none"></span>
                                <span class='icon ing' style="background:url(/public/install/images/loading.png)"></span>
                                <span class='label'>正在解析小程序代码</span>
                              </div>
                            </div>
                            <div class="list-item analysis-wxapp" style="display:none">
                               <div class="progress-label"></div>
                               <div class="progress-title">
                                <span class='icon over' style="display:none"></span>
                                <span class='icon ing' style="background:url(/public/install/images/loading.png)"></span>
                                <span class='label'>正在解析后台文件</span>
                              </div>
                            </div>
                            <div class="list-item analysis-view" style="display:none">
                              <div class="progress-label"></div>
                              <div class="progress-title">
                                <span class='icon over' style="display:none"></span>
                                <span class='icon ing' style="background:url(/public/install/images/loading.png)"></span>
                                <span class='label'>正在解析模板文件</span>
                              </div>
                            </div>
                            <div class="list-item analysis-model" style="display:none">
                              <div class="progress-label"></div>
                              <div class="progress-title">
                                <span class='icon over' style="display:none"></span>
                                <span class='icon ing' style="background:url(/public/install/images/loading.png)"></span>
                                <span class='label'>正在解析数据库</span>
                              </div>
                              <div class="progress-label"></div>
                            </div>
                            <div class="list-item analysis-sql" style="display:none">
                              <div class="progress-label"></div>
                              <div class="progress-title">
                                <span class='icon over' style="display:none"></span>
                                <span class='icon ing' style="background:url(/public/install/images/loading.png)"></span>
                                <span class='label'>正在安装数据库</span>
                              </div>
                            </div>
                            <div class="list-item zip-package" style="display:none">
                              <div class="progress-label"></div>
                              <div class="progress-title">
                                <span class='icon over' style="display:none"></span>
                                <span class='icon ing' style="background:url(/public/install/images/loading.png)"></span>
                                <span class='label'>正在压缩代码包</span>
                              </div>
                            </div>
                            <div class="list-item setup-package" style="display:none">
                              <div class="progress-label"></div>
                              <div class="progress-title">
                                <span class='icon over' style="display:none"></span>
                                <span class='icon ing' style="background:url(/public/install/images/loading.png)"></span>
                                <span class='label'>正在安装应用</span>
                              </div>
                            </div>
                         </div>
                    </div>
...
<script>
    ...
    $('.setup-btn').on('click',function(){
        ...
        let code = $('#install-code').val();
        let host = $('#db-host').val();
        let name = $('#db-name').val();
        let user = $('#db-user').val();
        let passwd = $('#db-passwd').val();

        if(code && host && name && user && passwd){
            layer.confirm('将执行安装程序，请不要关闭或刷新页面', {
                btn: ['确定','取消'] //按钮
            }, function(index){
                layer.close(index);
                $('.show-process').show();
                $('.progress-header-title').text("正在安装");
                analysisApplet(code, host, name, user, passwd);
            });
        }else{
            alert("请检查信息是否填写完整");
        }
    ...
    function analysisApplet(code, host, name, user, passwd) {
        $('.analysis-applet').show();
        $('.analysis-applet .label').css('color', '#5183fc');
        $.ajax({
            'type'  : 'post',
            'url'   : 'https://www.tiandiantong.com/wxapp/install/analysisApplet',
            'data'  : {code, host, name, user, passwd},
            'dataType' : 'jsonp',
            success : function(data){
                if(data.ec == 200){
                    $('.analysis-applet .label').text("小程序代码解析完成");
                    $('.analysis-applet .label').css('color', '#aaa');
                    $('.progress-num').text('6');
                    $('.precent-wrap .precent-bg').css('width', '6%');
                    analysisWxapp(code, host, name, user, passwd);
                }else{
                    $('.analysis-applet .label').text("小程序代码解析失败:"+data.em).css("color","red");
                    $('.analysis-applet .icon.over').css('border', '1px solid red');
                    layer.msg(data.em)
                }
                $('.analysis-applet .icon.over').show();
                $('.analysis-applet .icon.ing').hide();
            }
        });
    }

    function analysisWxapp(code, host, name, user, passwd) {
        $('.analysis-wxapp').show();
        $('.analysis-wxapp .label').css('color', '#5183fc');
        $.ajax({
            'type'  : 'post',
            'url'   : 'https://www.tiandiantong.com/wxapp/install/analysisWxapp',
            'data'  : {code, host, name, user, passwd},
            'dataType' : 'jsonp',
            success : function(data){
                if(data.ec == 200){
                    $('.analysis-wxapp .label').text("后台文件解析完成");
                    $('.analysis-wxapp .label').css('color', '#aaa');
                    $('.progress-num').text('40');
                    $('.precent-wrap .precent-bg').css('width', '40%');
                    analysisView(code, host, name, user, passwd);
                }else{
                    $('.analysis-wxapp .label').text("后台文件解析失败:"+data.em).css("color","red");
                    $('.analysis-wxapp .icon.over').css('border', '1px solid red');
                    layer.msg(data.em)
                }
                $('.analysis-wxapp .icon.over').show();
                $('.analysis-wxapp .icon.ing').hide();
            }
        });
    }

    function analysisView(code, host, name, user, passwd) {
        $('.analysis-view').show();
        $('.analysis-view .label').css('color', '#5183fc');
        $.ajax({
            'type'  : 'post',
            'url'   : 'https://www.tiandiantong.com/wxapp/install/analysisView',
            'data'  : {code, host, name, user, passwd},
            'dataType' : 'jsonp',
            success : function(data){
                if(data.ec == 200){
                    $('.analysis-view .label').text("模板文件解析完成");
                    $('.analysis-view .label').css('color', '#aaa');
                    $('.progress-num').text('50');
                    $('.precent-wrap .precent-bg').css('width', '50%');
                    analysisModel(code, host, name, user, passwd);
                }else{
                    $('.analysis-view .label').text("模板文件解析失败:"+data.em).css("color","red");
                    $('.analysis-view .icon.over').css('border', '1px solid red');
                    layer.msg(data.em)
                }
                $('.analysis-view .icon.over').show();
                $('.analysis-view .icon.ing').hide();
            }
        });
    }

    function analysisModel(code, host, name, user, passwd) {
        $('.analysis-model').show();
        $('.analysis-model .label').css('color', '#5183fc');
        $.ajax({
            'type'  : 'post',
            'url'   : 'https://www.tiandiantong.com/wxapp/install/analysisModel',
            'data'  : {code, host, name, user, passwd},
            'dataType' : 'jsonp',
            success : function(data){
                if(data.ec == 200){
                    $('.analysis-model .label').text("数据库解析完成");
                    $('.analysis-model .label').css('color', '#aaa');
                    $('.progress-num').text('60');
                    $('.precent-wrap .precent-bg').css('width', '60%');
                    analysisMysql(code, host, name, user, passwd);
                }else{
                    $('.analysis-model .label').text("数据库解析失败:"+data.em).css("color","red");
                    $('.analysis-model .icon.over').css('border', '1px solid red');
                    layer.msg(data.em)
                }
                $('.analysis-model .icon.over').show();
                $('.analysis-model .icon.ing').hide();
            }
        });
    }

    function analysisMysql(code, host, name, user, passwd) {
        $('.analysis-sql').show();
        $('.analysis-sql .label').css('color', '#5183fc');
        $.ajax({
            'type'  : 'post',
            'url'   : 'https://www.tiandiantong.com/wxapp/install/analysisMysql',
            'data'  : {code, host, name, user, passwd},
            'dataType' : 'jsonp',
            success : function(data){
                if(data.ec == 200){
                    $('.analysis-sql .label').text("数据库安装完成");
                    $('.analysis-sql .label').css('color', '#aaa');
                    $('.progress-num').text('70');
                    $('.precent-wrap .precent-bg').css('width', '70%');
                    analysisZip(code, host, name, user, passwd);
                }else{
                    $('.analysis-sql .label').text("数据库安装失败:"+data.em).css("color","red");
                    $('.analysis-sql .icon.over').css('border', '1px solid red');
                    layer.msg(data.em)
                }
                $('.analysis-sql .icon.over').show();
                $('.analysis-sql .icon.ing').hide();
            }
        });
    }

    function analysisZip(code, host, name, user, passwd) {
        $('.zip-package').show();
        $('.zip-package .label').css('color', '#5183fc');
        $.ajax({
            'type'  : 'post',
            'url'   : 'https://www.tiandiantong.com/wxapp/install/zipPackage',
            'data'  : {code, host, name, user, passwd},
            'dataType' : 'jsonp',
            success : function(data){
                if(data.ec == 200){
                    $('.zip-package .label').text("代码包压缩完成");
                    $('.zip-package .label').css('color', '#aaa');
                    $('.progress-num').text('80');
                    $('.precent-wrap .precent-bg').css('width', '80%');
                    analysisSetup(code, data.zipname, data.appletType, host, name, user, passwd);
                }else{
                    $('.zip-package .label').text("代码包压缩失败:"+data.em).css("color","red");
                    $('.zip-package .icon.over').css('border', '1px solid red');
                    layer.msg(data.em)
                }
                $('.zip-package .icon.over').show();
                $('.zip-package .icon.ing').hide();
            }
        });
    }

    function analysisSetup(code, zipname, appletType, host, name, user, passwd, shoudong) {
        $('.setup-package').show();
        $('.setup-package .label').css('color', '#5183fc');
        $.ajax({
            'type'  : 'post',
            'url'   : '/install/setup',
            'data'  : {zipname, host, name, user, passwd, shoudong},
            'dataType' : 'json',
            success : function(data){
                if(data.ec == 200){
                    $('.setup-package .label').text("应用安装完成");
                    $('.setup-package .label').css('color', '#aaa');
                    $('.progress-num').text('100');
                    $('.precent-wrap .precent-bg').css('width', '100%');
                    delZipFile(zipname, appletType);
                }else{
                    $('.setup-package .label').text("应用安装失败:"+data.em).css("color","red");
                    $('.setup-package .icon.over').css('border', '1px solid red');
                    layer.msg(data.em)
                }
                $('.setup-package .icon.over').show();
                $('.setup-package .icon.ing').hide();
            },
          	error: function(err){
              $('.setup-package .icon.over').css('border', '1px solid red');
              layer.confirm('手动下载 https://www.tiandiantong.com/public/build/'+zipname+", 放置到根目录下的installcode文件夹下，并重命名为tdtcode.zip，点击开始安装", {
                btn: ['开始安装','取消'] //按钮
              }, function(){
                layer.load(1, {
                  shade: [0.1,'#fff']
                });
                analysisSetup(code, zipname, appletType, host, name, user, passwd, 1)
              })
            }
        });
    }
  
    function delZipFile(zipname, appletType){
       $.ajax({
            'type'  : 'post',
            'url'   : 'https://www.tiandiantong.com/wxapp/install/delZip',
            'data'  : {zipname},
            'dataType' : 'jsonp',
            success : function(data){
                layer.msg("应用安装完成");
                window.location.replace("/install/setting?appletType="+appletType);
            }
        });
    }
</script>
