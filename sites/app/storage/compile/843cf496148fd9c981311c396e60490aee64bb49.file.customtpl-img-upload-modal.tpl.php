<?php /* Smarty version Smarty-3.1.17, created on 2020-04-02 09:32:30
         compiled from "/www/wwwroot/default/yingxiaosc/yingxiaosc/sites/app/view/template/wxapp/customtpl-img-upload-modal.tpl" */ ?>
<?php /*%%SmartyHeaderCode:15148266825e8540aebb71e5-63261603%%*/if(!defined('SMARTY_DIR')) exit('no direct access allowed');
$_valid = $_smarty_tpl->decodeProperties(array (
  'file_dependency' => 
  array (
    '843cf496148fd9c981311c396e60490aee64bb49' => 
    array (
      0 => '/www/wwwroot/default/yingxiaosc/yingxiaosc/sites/app/view/template/wxapp/customtpl-img-upload-modal.tpl',
      1 => 1575621712,
      2 => 'file',
    ),
  ),
  'nocache_hash' => '15148266825e8540aebb71e5-63261603',
  'function' => 
  array (
  ),
  'variables' => 
  ...
  'unifunc' => 'content_5e8540aebc8108_67148932',
),false); /*/%%SmartyHeaderCode%%*/?>
<?php if ($_valid && !is_callable('content_5e8540aebc8108_67148932')) {function content_5e8540aebc8108_67148932($_smarty_tpl) {?><!--<link rel="stylesheet" href="/public/site/css/help/swiper-4.2.2.min.css">-->
<style>
    #myimg{
        font-family: "黑体";
    }
...
//			    });
    ...
    function toUpload(elem){
        ...
        if($(elem).data('type') == 'slide'){
            var ratio   = $(elem).data('ratio');
            width   = Math.floor((278-parseInt($(elem).parent().css('padding-left'))-parseInt($(elem).parent().css('padding-right')))/ratio)*2;
            height  = parseInt(parseInt($(elem).css('height'))/ratio*2);
        }else if($(elem).data('type') == 'title'){
            var ratio   = $(elem).data('ratio');
            width = $(elem).data('width');
            var oriHeight = parseInt($(elem).parent().css('height'));
            if($(elem).data('btn') == 'text'){
                oriHeight = parseInt($(elem).prev().css('height'));
            }
            height = parseInt(oriHeight/ratio*2);
        }else if($(elem).data('type') == 'video'){
            var ratio   = $(elem).data('ratio');
            width = $(elem).data('width');
            var oriHeight = parseInt($(elem).css('height'));
            height = parseInt(oriHeight/ratio)*2;
        }else if($(elem).data('type') == 'pictext'){
            var ratio   = $(elem).data('ratio');
            width   = parseInt(parseInt($(elem).parent().css('width'))/ratio*2);
            height  = parseInt(parseInt($(elem).parent().css('height'))/ratio*2);
        }else if($(elem).data('type') == 'image'){
            var ratio   = $(elem).data('ratio');
            console.log($(elem).css('width'));
            width   = Math.round((parseFloat($(elem).css('width'))/ratio*2));
            height  = Math.round((parseFloat(window.getComputedStyle(elem).height)/ratio*2));
        }else if($(elem).data('type') == 'window'){
            var ratio   = $(elem).data('ratio');
            width   = Math.round((parseFloat($(elem).css('width'))/ratio));
            height  = Math.round((parseFloat(window.getComputedStyle(elem).height)/ratio));
        }else if($(elem).data('height') && $(elem).data('width')){
            width = $(elem).data('width');
    ...
    function initUploadBox(){
        var html = '<div class="cropper-box" data-width="'+width+'" style="width: 250px;display: inline-block; margin-right: 50px" data-height="'+height+'">';
    ...
    function uploadGif(e){
        ...
        if(isImageFile){
            var url = "/admin/image/upload/common/default?dir=image",
                data = new FormData();
