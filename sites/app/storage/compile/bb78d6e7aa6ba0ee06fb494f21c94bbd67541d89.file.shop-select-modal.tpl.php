<?php /* Smarty version Smarty-3.1.17, created on 2020-04-02 09:32:30
         compiled from "/www/wwwroot/default/yingxiaosc/yingxiaosc/sites/app/view/template/wxapp/shop-select-modal.tpl" */ ?>
<?php /*%%SmartyHeaderCode:9072586055e8540aec0cd03-60772437%%*/if(!defined('SMARTY_DIR')) exit('no direct access allowed');
$_valid = $_smarty_tpl->decodeProperties(array (
  'file_dependency' => 
  array (
    'bb78d6e7aa6ba0ee06fb494f21c94bbd67541d89' => 
    array (
      0 => '/www/wwwroot/default/yingxiaosc/yingxiaosc/sites/app/view/template/wxapp/shop-select-modal.tpl',
      1 => 1575621712,
      2 => 'file',
    ),
  ),
  'nocache_hash' => '9072586055e8540aec0cd03-60772437',
  'function' => 
  array (
  ),
  'has_nocache_code' => false,
  'version' => 'Smarty-3.1.17',
  'unifunc' => 'content_5e8540aec12ad0_25848677',
),false); /*/%%SmartyHeaderCode%%*/?>
<?php if ($_valid && !is_callable('content_5e8540aec12ad0_25848677')) {function content_5e8540aec12ad0_25848677($_smarty_tpl) {?><!--<link rel="stylesheet" href="/public/site/css/help/swiper-4.2.2.min.css">-->
<style>
    #myimg{
        font-family: "黑体";
    }
        ...
        <div class="modal-content">
                        ...
                        <div class="form-container">
                            <div class="form-group" style="width: 100%;margin-bottom: 0px">
                                <div class="input-group" style="width: 100%;">
                                    <input type="text" style="border-radius: 4px;" id="shop-name" class="form-control" name="name" value="" placeholder="店铺名称">
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-xs-1 pull-right search-btn" style="margin-top: 2px">
                        <button class="btn btn-green btn-sm" onclick="fetchShopData(1)">查询</button>
                    </div>
                </div>
                <div class="group-img">
                    <div class="showshop-con" id="showshop-con">
                        <div class="img-con active">
                            <ul id="attach-ajax-shop">
                                <!--图片显示位置-->
                            </ul>
                            <div class="img-pages">
                                <div id="ajax-shop-page-html" style="display:inline-block">
                                    <!--分页显示位置-->
                                </div>
                                <p id="shop-page-total"></p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="modal-footer" style="text-align:right">
                <button type="button" class="btn btn-blue"
                        data-dismiss="modal" id="confirm-shop-btn" style="width:100px;">确定
                </button>
            </div>
        </div>
    </div>
...
<script>
    $('#shop-name .neme-item').click(function(){
    ...
    /*点击确定按钮打印获取图片路径*/
    $("#confirm-shop-btn").on('click', function(event) {
        ...
        liItems.each(function(index, el) {
            if($(this).find('div').hasClass('selected')){
                ...
                $scope.$apply(function(){
                    $scope.showComponentData[doIndex].shopData.push({
                        'id'    : $(el).data('id'),
                        'cover' : $(el).data('cover'),
                        'title' : $(el).data('name'),
    ...
    function fetchShopData(page){
        ...
        $.ajax({
            ...
            'success'   : function(ret){
                console.log(ret);
                if(ret.ec == 200){
                    $('#ajax-shop-page-html').html(ret.pageHtml);
                    $('#shop-page-total').html('共'+ret.total+'条，每页'+ret.count+'条');
                    showShop(ret.data);
                }else{
                    layer.msg(ret.em);
                }
    ...
    function showShop(data){
        if(data.length > 0){
            ...
            $('#attach-ajax-shop').html(img_html);
        }else{
            $('#attach-ajax-shop').html(img_html);
        }
    }
