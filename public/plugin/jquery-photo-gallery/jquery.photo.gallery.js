...
$.extend({
	...
	openPhotoGallery : function(obj){
		...
		var img = $img[0],
			...
			winHeight,
			winWidth,
			maxHeight = document.getElementById("main-container").offsetHeight - windowMargin * 2 - 45,
		...
		$("<iframe></iframe").appendTo("body")
			.attr("id", "J_pg")
			.attr("src", "/public/plugin/jquery-photo-gallery/gallery.html")
			.css({
				position : "absolute",
				left : (document.body.clientWidth - winWidth) /2,
				top : (document.getElementById("main-container").offsetHeight + 45 - winHeight) /2,
				width : winWidth,
				height : winHeight,
