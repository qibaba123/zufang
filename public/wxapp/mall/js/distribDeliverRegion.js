...
app.controller('areaCtrl', ['$scope', '$http', function($scope, $http){
	...
	$scope.ladderToggle = function($event){
		...
		if(ulElem.hasClass('area-editor-depth0')){
			...
			if(_this.hasClass('extend')){
				_this.parents('.area-editor-wrap').find('.area-editor-depth0').each(function(index, el) {
					var ulThis = $(this);
					console.log(ulThis.children('li').eq(parentLiIndex).find('.area-editor-depth1').length);
	...
	$scope.selectToggle = function($event,item){
		...
		if(_this.hasClass('area-editor-list-select')){
			...
			console.log(waitobj);
			console.log($scope.waitSelectList);
		}else{
			var waitobj = $scope.getWaitObj(type,item);
			waitobj.action = 'add';
			console.log(waitobj);
			...
			if(item.action=='add'){
				switch(item.type){
					...
					case 'c':
						...
						break;
					case 'a':
						console.log(item.pid+"/"+item.cid+"/"+item.aid)
	...
	$scope.deleteSelectItem = function(event,item){
		...
		console.log($scope.selectAreaList);
	} 
	//选中某省下所有区域 参数为省的索引
	...
	$scope.selectAllAreasByC = function(p_id,c_id){
		...
		// 要选中的城市
		// 设置城市的显示区域为该城市下所有的区域数目
		console.log($scope.selectAreaList[p_id].region.state[c_id].cityShow);
		...
		console.log("城市选中数量"+cityShowAllNum);
	...
	$scope.selectAllAreasByA = function(p_id,c_id,a_id){
		console.log($scope.selectAreaList[p_id].region.state[c_id]);
	...
	$scope.unselectAllAreaByA = function(p_id,c_id,a_id){
		console.log($scope.selectAreaList[p_id].region.state[c_id].city[a_id]);
	...
	$scope.saveTemplate = function(){
		...
		if(data.name && data.type){
			...
			$http({
				method: 'POST',
				url:    '/distrib/delivery/saveTemplate',
				data:   data
			}).then(function(response) {
				layer.close(index);
				layer.msg(response.data.em);
				window.location.href='/distrib/delivery/index';
			});
		}else{
			layer.msg("请完善模板信息");
		}
