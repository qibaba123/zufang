...
$("input[name='no_express']").click(function(event) {
    ...
    if(isChecked && value=='0'){
        ...
        $("#shop-sned-info").stop().hide();
        $("#shop-express-note").stop().show();
    }else if(isChecked && value=='2'){
        $("#wuliu-info").stop().hide();
        $("#shop-sned-info").stop().show();
        $("#shop-express-note").stop().hide();
    }else{
 ...
 * 订餐配送逻辑处理
 */
function expressMeal(){
    var tid = $('#hid_id').val();
    var need        = 1;
    var company     = $('#courier_name').val();
    var code        = $('#courier_phone').val();
    if(need == 1 && !(company && code)){
...
function expressTrade(){
    ...
    var expressNote = $('#express_note').val();
    if(need == 1 && !(express && code)){
        layer.msg("请选择物流并填写物流单好");
        return false;
    }

    if(need == 2 && !(deliveryName && deliveryMobile)){
        layer.msg("请填写配送员信息");
        return false;
    }
    if(tid){
        ...
        var data = {
            'tid'       : tid,
            'need'   	: need,
            'company'   : need==1?company:deliveryName,
            'express'   : express,
            'code' 		: need==1?code:deliveryMobile,
