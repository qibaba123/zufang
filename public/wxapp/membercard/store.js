...
function fetchPageData(page){
    ...
    $.ajax({
        'type'  : 'post',
        'url'   : '/wxapp/membercard/ajaxStoreVerify',
        'data'  : data,
        'dataType' : 'json',
...
function showVerifyHtml(data){
    ...
    for(var i=0; i < data.length ; i++){
        ...
        _html += '<td>'+data[i].om_card_num+'</td>';
        _html += '<td>'+data[i].m_mobile+'</td>';
        _html += '<td>'+data[i].storeName+'</td>';
        _html += '<td>'+temp+'</td>';
        _html += '</tr>';
    }
...
function verify(data){
    ...
    $.ajax({
        'type'  : 'post',
        'url'   : '/wxapp/membercard/saveVerify',
        'data'  : data,
        'dataType' : 'json',
        'success'   : function(ret){
            layer.close(index);
...
function saveStore(){
    ...
    var data = {};
    var check = new Array('name','province','city','zone','addr','contact','lng','lat','open_time','close_time','logo','cover','feature','recommend');
    ...
    $.ajax({
        'type'   : 'post',
        'url'   : '/wxapp/membercard/saveStore',
        'data'  : data,
        'dataType'  : 'json',
        'success'   : function(ret){
            layer.close(index);
            if(ret.ec == 200){
                window.location.href='/wxapp/membercard/index';
            }else{
                layer.msg(ret.em);
            }
        }
...
function shopSaveStore(){
    ...
    $.ajax({
        'type'   : 'post',
        'url'   : '/shop/shop/saveOfflineStore',
        'data'  : data,
        'dataType'  : 'json',
        'success'   : function(ret){
            layer.close(index);
            if(ret.ec == 200){
                window.location.href='/shop/shop/offlineStoreList';
            }else{
                layer.msg(ret.em);
            }
        }
...
function initRegionByName(data){
    if(data){
        $.ajax({
            'type'   : 'post',
            'url'   : '/wxapp/membercard/dealRegionByName',
            'data'  : data,
            'dataType'  : 'json',
            'success'   : function(ret){
...
function get_member_card(data){
    ...
    $.ajax({
        'type'   : 'post',
        'url'   : '/wxapp/membercard/memberInfo',
        'data'  : data,
        'dataType'  : 'json',
        'success'   : function(ret){
            if(ret.ec == 200){
