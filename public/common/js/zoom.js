(function($) {
	...
	function openPrevious() {
		var prev = openedImage.parent('div.gallery-div').prev('.gallery-div');
		if (prev.length == 0) {
			prev = $('.gallery div.gallery-div:last-child');
		}
		prev.find('a.gallery-a').trigger('click');
	}
	
	function openNext() {
		var next = openedImage.parent('div.gallery-div').next('.gallery-div');
		if (next.length == 0) {
			next = $('.gallery div.gallery-div:first-child');
		}
		next.children('a.gallery-a').trigger('click');
	}
	...
	(function bindNavigation() {
		...
		*/
		$('.zoom, .gallery a.gallery-a').on('click', open);
	})();
